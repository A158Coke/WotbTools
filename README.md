...
- **陪练与打手申请**：`?view=boost` 页面支持玩家提交打手资格申请，管理员在资格审批中查看截图、联系方式和在线时间，审批通过后自动关联 Keycloak `booster` role 与打手资料。数据库约束会先 flush，realm role 变更在事务回滚时自动补偿。打手管理页会分别显示资格状态（`status`）和接单状态（由 `available + activeAssignmentCount` 推导为可接单/忙碌/暂停接单），分配时会优先推荐可接单且匹配度更高的打手；打手本人可在工作台接单、开始、提交完成或拒单，个人中心可回看历史订单，并通过站内通知提醒相关用户。
