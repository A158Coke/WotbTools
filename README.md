...
| 版本          | 技术栈                                        | 入口                                                   | 适用场景                     |
|-------------|--------------------------------------------|------------------------------------------------------|--------------------------|
| Java Web 版  | Java 21 + Spring Boot 4 + Vue 3 + Docker | `docker\online\` 本地开发；CI/CD → GHCR `backend`/`frontend`/`keycloak` 三镜像 | 浏览器上传、在线预览、排行榜、REST API、Keycloak 认证 |
...
- **陪练与打手申请**：`?view=boost` 页面支持玩家提交打手资格申请，管理员在资格审批中默认查看最新待审批记录，并可点击截图站内放大；审批通过后自动关联 Keycloak `booster` role 与打手资料。数据库约束会先 flush，realm role 变更在事务回滚时自动补偿。打手管理页会分别显示资格状态（`status`）和接单状态（由 `available + activeAssignmentCount` 推导为可接单/忙碌/暂停接单），分配时会优先推荐可接单且匹配度更高的打手；打手本人可在工作台接单、开始、提交完成或拒单，个人中心可回看历史订单，并通过站内通知提醒相关用户。
...
需要 JDK 21、Maven、Node.js；完整运行还需要 Docker Desktop。
...
> 后端始终依赖 PostgreSQL、Flyway 与 Keycloak，不能在无数据库环境直接运行 JAR。Windows 默认 `java` 也可能是 JDK 8，执行 Maven 前先把 `JAVA_HOME` 指向 JDK 21。
...
- 部署前自动备份 `wotb` 与 `keycloak`；`database-backup.yml` 每日香港时间 03:15 再备份一次。
- 归档分别保存在 `/opt/wotb/backups/wotb/`、`/opt/wotb/backups/keycloak/`，完整读取校验后保留 7 天。
- 恢复只允许 SSH 手动执行 `deploy/postgres-restore.sh`，文件必须位于对应数据库目录，并显式传入 `--confirm RESTORE-wotb` 或 `RESTORE-keycloak`；恢复前会再做安全备份。
