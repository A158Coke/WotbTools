...
28. **战局回放坦克标记** — 开发 AI Review 战局回放/地图标记时，默认复用 `frontend/src/assets/tank-icons/tank-marker-{friendly,enemy}-{hull,turret}.png` 的通用半立体 MT 双层模型，并遵循 `docs/assets/battle-replay/tank-marker-authoritative-master.png` 同源基材、`docs/assets/battle-replay/tank-marker-state-spec.png` 规范表与 `frontend/src/assets/tank-icons/README.md` 契约：车体按 `hullYaw` 旋转、炮塔按 `turretWorldYaw = hullYaw + turretRelativeYaw` 整体旋转，炮管不得脱离炮塔独立旋转；轨迹只代表历史位置不代表朝向。该契约已于 2026-08-13 落地：`directionSamples{hullYawDeg,turretRelativeYawDeg}`（type-10 yaw + type-7 propId=2：u16*360/65536-180，炮口世界方向=normalize(hull+rel)，开火锚点交叉验证残差 2.3°）；BattlePlayback 已用四张运行时 PNG 渲染 HTML overlay 双层标记（hull 按 hullYawDeg、turret 按 turretWorldYawDeg 旋转，约 28px/移动端 22px，共同 pivot 256,256）。禁止退化为普通圆点、分别生成两层、运行时 CSS filter 换阵营色或绘制与现有资产风格不一致的新图标。
