...
28. **战局回放车型图标** — 开发 AI Review 战局回放/地图标记时，优先复用 `frontend/src/assets/tank-icons/` 中的轻坦、中坦、重坦、坦歼俯视 SVG；用 `currentColor` 表达阵营/状态，保持车头随行进方向旋转。禁止退化为普通圆点、重复绘制同类图标或生成与现有资产风格不一致的新图标。
