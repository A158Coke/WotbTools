...
28. **战局回放坦克标记** — 开发 AI Review 战局回放/地图标记时，默认复用 `frontend/src/assets/tank-icons/tank-marker-{hull,turret}.png` 的通用半立体 MT 双层模型，并遵循同目录状态规范表与 README：车体和完整炮塔分别旋转，炮管不得脱离炮塔独立旋转。四个车型 SVG 只作早期方案兼容，不得作为新实现默认标记。禁止退化为普通圆点、重复生成坦克素材或绘制与现有资产风格不一致的新图标。
