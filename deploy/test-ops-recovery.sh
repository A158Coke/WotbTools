...
! grep -Fq 'ref: ${{ inputs.target_sha || github.sha }}' "$workflow"
! grep -Fq 'ref: ${{ needs.prepare.outputs.target_sha }}' "$workflow"
grep -Fq 'git fetch origin main' "$workflow"
grep -Fq 'control_plane_sha="$(git rev-parse origin/main)"' "$workflow"
grep -Fq 'if [ "$source_sha" != "$control_plane_sha" ]; then' "$workflow"
grep -Fq 'control_plane_sha: ${{ steps.target.outputs.control_plane_sha }}' "$workflow"
grep -Fq 'ref: ${{ needs.prepare.outputs.control_plane_sha }}' "$workflow"
grep -q 'git ls-tree -r --name-only "\$target_sha"' "$workflow"
grep -q 'source: deploy' "$workflow"
echo "specific SHA uses historical source only for identity/migration lookup; current origin/main provides the control-plane"
