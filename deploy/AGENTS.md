...
- Grafana 生产 API 校验必须复用生产 backend Alpine 运行时可用的 BusyBox `wget`：在 backend 容器内生成 `Authorization: Basic` header（不得把密码放 URL、命令输出或日志），datasource health 的唯一成功值是 JSON `status=OK`。CI runtime smoke 还必须在 Alpine 3.22 中验证正确凭据通过、错误凭据失败，以防回退到 GNU-only `wget` 参数。
