...
- **Build identity（防猜版本）**：Build 的 `changes` job 只在 `main` 上解析一次 full commit SHA，并由 backend/frontend/keycloak 三个 job checkout 同一个 SHA；frontend 与 keycloak 构建都注入该 SHA 的 `BUILD_COMMIT` build-arg。Frontend 的 `vite.config.js` 据此生成 `dist/version.json`（`buildCommit`）并在启动 console 输出 `[build] commit=... time=...`；Keycloak 通过 `WOTBTOOLS_BUILD_COMMIT` 在启动时输出 `WotBTools Keycloak build=...`（Docker 上下文无 `.git`，必须经 build-arg 注入，本地前端构建才 fallback `git rev-parse`）。生产页面/认证异常时先核对实际 bundle 与 Keycloak 日志版本。
