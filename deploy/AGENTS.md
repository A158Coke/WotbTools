...
- 增量部署：deploy.yml 按变更路径只重建对应镜像（backend ← java/ + common tier 文件 + rating/map_names/map-semantics；frontend ← frontend/ + map_names + assets + nginx；keycloak ← docker/keycloak + Dockerfile.keycloak）；改动此处路径匹配时同步 workflow。
- 生产编排 `deploy/docker-compose.prod.yml` + `deploy/deploy.sh`（fail-fast 校验，含 `AI_REVIEW_WORKER_OVERALL_DEADLINE_SEC=1100` 等契约；改动后端超时/编排变量必须同步 `AiTimeoutChainContractTest`、`.env.example` 与本文件）。
...
- secret 一律 GitHub Secrets / 运行时 env（`.env.example` 只列变量名），禁止落库或写死；赞助/收款信息不硬编码进页面或仓库（运行时只读挂载）。