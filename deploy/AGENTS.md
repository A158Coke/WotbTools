...
- **Build / Deploy 分离**：`.github/workflows/build.yml` 在 main 的应用或运行时 observability 变更后构建 component-local 的 SHA 与 `latest` 镜像 tag，也支持 `workflow_dispatch` 单独构建 `backend` / `frontend` / `keycloak` / `all`；`changes` job 只解析一次事件携带的 full commit SHA，三个 builder checkout 同一个冻结 SHA，禁止 feature ref 或移动的 main 写入自动 production manifest，手工 Build 还必须验证 commit 已在 `origin/main` 历史中。纯 `deploy/observability/grafana/dashboards/**` 由 Grafana OpenTofu 管理，不触发应用 Build。成功的 main Build 通过 `workflow_run` 自动接力 `.github/workflows/deploy.yml`，Deploy 的手工入口只接受 `all` / `backend` / `frontend` / `keycloak` 应用入口；自动 Deploy 只消费对应 Build 的 immutable manifest，应用服务/all 必须使用 manifest 指定的 `sha-<first-12-sha>` tag，手工应用发布才使用对应组件 `latest`，运行时 observability service 不属于普通 Deploy UI。代码质量验证由 PR CI（merge gate）承担，Build/Deploy 不重复运行测试套件。targeted deploy 只重建并启动所选 service，保留其它应用镜像 tag，且不提升 LKG；targeted failure 只恢复该 service 的 pre-deploy snapshot，完整自动 `all` 发布才更新或从 LKG 回滚。部署 generation 状态按 service 分文件记录，并在任何生产 mutation 前完成所有目标 service 的 stale precheck。
...
- **Build identity（防猜版本）**：Build 的 `changes` job 只在 `main` 上解析一次 full commit SHA，并由 backend/frontend/keycloak 三个 job checkout 同一个 SHA；frontend 构建注入该 SHA 的 `BUILD_COMMIT` build-arg，`vite.config.js` 据此生成 `dist/version.json` 并在启动 console 输出 `[build] commit=... time=...`（Docker 上下文无 `.git`，必须经 build-arg 注入，本地构建才 fallback `git rev-parse`）。生产页面异常时先核对实际 bundle 版本。
...
- 生产发布必须先由 Actions 上传到 `/opt/wotb/deploy.incoming`，在 incoming project root 中完成 compose config/pull，再以同文件系统目录 move promote 到 `/opt/wotb/deploy`；自动成功完整部署后将通过 application availability gate 的部署树、compose 与 SHA 提升为 `/opt/wotb/deploy.lkg`、`docker-compose.lkg.yml`、`DEPLOYED_SHA.lkg`。手工 `latest` 应用发布只更新三项应用，不把可变 `latest` 写入 LKG。完整自动 `all` 失败只允许恢复经校验的 LKG；targeted 失败恢复当前 `deploy.prev` / `docker-compose.prev.yml` snapshot 并只重建失败 service，绝不得回滚不相关的独立 targeted release。LKG 缺失/损坏时必须 fail-closed；若存在健康 live deployment，正常流程验证并 seed LKG；不存在健康 live deployment 时不得 bootstrap candidate。应用可用性与观测可用性是两个故障域；Prometheus/Loki/Alloy/Grafana 失败只能输出 `OBSERVABILITY DEGRADED`，不得触发健康应用回滚。COS production OpenTofu Apply 与 production deploy、Grafana Apply 共用 GitHub Actions `production-maintenance` concurrency，但该组不是服务器端 distributed lock。禁止对 live `deploy/` 直接 SCP 覆盖，也禁止只发送 HUP 作为 Grafana/观测配置生效保证。
