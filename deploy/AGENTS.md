...
- 生产发布必须先由 Actions 上传到 `/opt/wotb/deploy.incoming`，在 incoming project root 中完成 compose config/pull，再以同文件系统目录 move promote 到 `/opt/wotb/deploy`；成功部署后将整棵已验证部署树、compose 与 SHA 提升为 `/opt/wotb/deploy.lkg`、`docker-compose.lkg.yml`、`DEPLOYED_SHA.lkg`。失败时只允许恢复经校验的 LKG；`/opt/wotb/deploy.prev` 与 `docker-compose.prev.yml` 仅是取证快照。LKG 缺失/损坏时必须 fail-closed 并保留当前 live tree，只有显式 `workflow_dispatch` bootstrap 才能首次建立 LKG。回滚后仍须 `--force-recreate` Prometheus/Loki/Alloy/Grafana 并重新跑完整 observability gate。禁止对 live `deploy/` 直接 SCP 覆盖，也禁止只发送 HUP 作为 Grafana/观测配置生效保证。
...
- Keycloak 运行时契约由 `deploy/test-keycloak-runtime.sh` 独立验证：自定义镜像必须以 `start --optimized` 启动，构建时启用 PostgreSQL/health/metrics/histograms，应用 discovery、`:9000/health/ready`、`:9000/metrics` 必须成功，management 端口不得映射到宿主机，且不得出现启动时 Quarkus augmentation。`.github/workflows/ci.yml` 的 `keycloak-runtime` 是该契约的真实 Docker gate。
