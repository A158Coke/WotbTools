...
### Fixed
- 美化回放解析上传区：新增页面说明、能力标签、居中拖拽卡片和已选文件状态，修复上传页可见乱码符号。
- 修复中文 locale 被错误编码导致回放、排行榜、陪练、扩展分析页面中文乱码的问题，并补齐陪练表单 placeholder 的三语文案。
- 首页卡片、标签、页脚改为三语 i18n，并修复非管理员用户看到用户管理入口的问题。
- 前端 nginx 增加 UTF-8 charset，避免静态资源响应头缺失字符集。
