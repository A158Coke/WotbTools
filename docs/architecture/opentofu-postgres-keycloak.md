...
Keycloak PostgreSQL application role, database, and database grant. It remains
separate from the Keycloak application runtime and uses the
`tofu_state.tofu_keycloak_postgres` schema on Business PostgreSQL for state.
...
2. A merged main change is staged into an immutable SHA-named directory
   on TX through SSH.
3. GitHub Actions injects the dedicated `PG*` backend variables and `TF_VAR_*`
   provider variables over SSH; TX initializes the PostgreSQL backend, creates one saved plan
...
`tofu_state.tofu_keycloak_postgres`; operator coordination is not a
substitute for the workflow-level lock.

GitHub Actions Secrets/Variables are the sole TX configuration entry point for
the PostgreSQL backend credentials, provider administrator password, Keycloak role
