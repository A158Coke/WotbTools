...
- Current Web/Android DTOs keep their existing status vocabularies, implemented by the three real enums `ReplayProcessingJob.Status` (`QUEUED/PROCESSING/READY/FAILED/CANCELLED`), `ExportJob.Status` (`QUEUED/PROCESSING/READY/FAILED/CANCELLED`) and `ReplayProcessingJob.SourceStatus` (`PENDING/PROCESSING/READY/FAILED`). The explicit `CurrentProcessingStatusAdapter` lives in `wotb-web` and binds directly to all three enums; it never merges job `QUEUED` with source `PENDING`, and rejects future `CANCELLED` when the target source contract cannot represent it. No current public DTO imports `JobStatus`.
