...
- **掉血窗口严重度**：`DamageWindowClusterer.DamageWindow` 带 `damageVsEntryMaxHpPct`（累计伤害/进场满血量——优先回放实测 `observedMaxHp`、含装备/物资加成，无实测回退 tankopedia 基础值；只是计算基准，不是实际掉血比例）：跨度 ≤10s 且伤害 ≥75% 进场满血量 → `criticalWindow`（短窗高额伤害窗口）；不判定「被秒杀」（无法证明窗口起始血量、窗口内阵亡与精确进场血量）；证据段输出基准百分比与标记，prompt 规则（player×3 + team/single 三语）强制定性并给时间范围。
