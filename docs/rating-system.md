...
`GET /api/rating` 返回当前评分参数快照（`Rating.config()` → `RatingConfig` record）：权重 `assist/block/killValue/winBonus`、`minSamples`、`scale`、车型系数 `classFactor`。改 `common/rating.json` 后接口与前端弹窗自动反映，无需改代码。
