...
- **AI 输出语言跟随前端 locale**：`/api/replay/analyze` 新增 multipart 表单字段 `lang`（白名单 `zh`/`en`/`ru`，缺失或未知返回 400 `UNKNOWN_LOCALE`）。语言穿透 ReviewService → facade → Player/Team Service → Prompt Builder，在 system prompt 末尾追加「输出语言 + 时间格式」指令（zh 指令为空，现有中文输出字节不变；en 时间格式 `Xm Xs`，ru 为 `X мин X с`）。覆盖 player full/fallback/multi 与 team single/multi 全部路径；地图/坦克/clan/昵称等专有名词不翻译；`limitations` 与错误码仍为英文稳定码、由前端本地化。前端由 vue-i18n 当前 locale 携带 `lang`。
