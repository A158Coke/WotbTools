...
- `BoostPage.vue` 的资格审批默认请求 `NEW` 状态，服务端按 `created_at DESC` 返回最新待审批申请。`GET /api/admin/boost/booster-applications` 与 `GET /api/boost/booster-applications/my` 使用 `BoosterApplicationSummaryDto` 构造投影，数据库查询不得读取 `overall_stats_image` / `vehicle_stats_image`；`reviewing` / `approve` / `reject` 三个 PATCH 端点也返回摘要 DTO。管理员点击“详情”后才调用 `GET /api/admin/boost/booster-applications/{id}` 获取完整资料和 Base64 截图。前端同一时间只保留一条详情，缩略图使用 `loading="lazy"` + `decoding="async"`，大图继续使用站内遮罩预览。
