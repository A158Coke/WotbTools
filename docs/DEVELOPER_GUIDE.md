...
`deployment-manifest`；backend/frontend/keycloak 各只构建一次并先发布 GHCR，然后经既有 TX SCP/SSH access pattern 调用 TX-host-only `deploy/tx/replicate-image-to-tcr.sh`。该 helper 只接受 component allowlist 与 immutable `sha-<12>` tag，并使用 TX host 已有的 deployment-owned Docker credential store：拉取 GHCR immutable image、推送同一 immutable image 到 Tencent TCR、读取两端的**registry manifest digest**并要求相等，最后才更新 TCR `latest`。`latest` 只同步已验证 immutable artifact，不能作为 deploy identity；任何 GHCR pull、TCR push、digest lookup/mismatch 或 latest 更新失败都会使 Build 失败、阻止 manifest。此前 GitHub-hosted `crane copy` 在 run `35603682885` 中对 Frontend 与 Keycloak 均在 600 秒超时，故不得声称 runner-side copy 避开跨区域 blob transfer。**GHCR 是 source/recovery registry，Tencent TCR 是 TX runtime image source**；TCR credential 不经 SSH、Compose、metadata 或 application environment 传递。MinIO Dockerfile 改动还会构建同样 immutable tag 的源码固定
