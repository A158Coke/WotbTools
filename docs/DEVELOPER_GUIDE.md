...
5. 每次成功部署先把完整已验证部署树提升为 `/opt/wotb/deploy.lkg`、`docker-compose.lkg.yml` 与 `DEPLOYED_SHA.lkg`；健康检查失败只从该 LKG 恢复并再次验证，`deploy.prev` 仅供取证。
...
8. 部署前保存 `deploy.prev` 取证快照；compose 切换后显式 reload Prometheus/Alloy，观测 gate 验证 Prometheus `up == 1` 与 deployment-specific Loki canary，失败时从 LKG 同时恢复 compose 和 observability 配置。没有经校验的 LKG 时禁止破坏当前 live tree；只有显式 `workflow_dispatch` bootstrap 输入允许首次建立 LKG。若历史 live Alloy 已因旧 selector 进入重启循环，初始 LKG 会记录为 `bootstrap-baseline`，不能冒充 fully validated LKG；完整 observability gate 在 promote 后执行。回滚使用当前 incoming release 持有的 verifier，历史 LKG 缺少该脚本时不会阻断恢复，并依据 `DEPLOYED_CAPABILITIES.lkg` 只执行该 LKG 已声明的 capability gate。
