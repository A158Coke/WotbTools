...
| `TeamAutopsyService` | `wotb-web/.../ai/TeamAutopsyService.java` | 第 3 次调用（仅团队复盘）：判负→主要战犯 / 判胜→MVP（预算按整体剩余裁剪，失败不影响主复盘） |
...
- **Team Autopsy（第 3 次调用，仅 team perspective）**：战犯/MVP 只应用于训练房/联赛团队复盘（`TeamReplayAnalysisService` 单团队单元），随机战斗个人复盘（`TacticalReviewHarness`）不输出。玩家身份用 `playerKey`（P1..P7，本方 roster 稳定编号，同队同名坦克可区分）；Parser 严格验证 playerKey 归属/不重复、contribution/confidence 枚举、verdict 引用有效且 evidence 非空、判胜≥1 MVP / 判负≥1 战犯、空结果拒绝；渲染按 playerKey 回查后端权威昵称/坦克名。`TeamAutopsyStatsBuilder` 只构建 recorderTeam 本方玩家，weakOutput 均值仅本方；结算字段为 Battle Result 事实，earlyDeath/weakOutput 为规则候选（各自置信度），deathInCriticalWindow 继承窗口 confidence 且结算级代理不得 EXACT；死亡时间线仅本方。Call #3 预算 = min(30s, 整体剩余 - safety margin)，不足不启动并记录 budget_exhausted；`AI_CANCELLED` 重新抛出。
