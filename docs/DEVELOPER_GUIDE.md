...
- **三环（Mark 3）**：`wotb-web/.../mark3/` 为独立的 Tier X 单车最速三环人工审核领域，不接 Wargaming API。玩家提交 `battleCount` / `averageDamage` / `winRate`（0–100，最多两位小数）、1–2 张截图和 5 个已验证回放；公开榜与管理筛选复用百场的国家/系别、车种、车辆交集。状态仅 PENDING/CURRENT/REJECTED/CANCELLED/DELETED（无 SUPERSEDED）：同用户同车的 CURRENT 唯一且不被后续申请替代，拒绝/撤销/删除后可重提；排名以批准的 battleCount 升序，场数相同为 competition rank，稳定展示按 `approvedAt ASC, id ASC`。审批端点无成绩请求体，管理员只能通过、拒绝或删除；终态清理 proof 和 replay evidence。详见 `docs/features/hall-of-fame.md`「三环」章节。
