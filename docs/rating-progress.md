...
扩展页、实时接口和目标算法主体已完成。`average_hp` 现由本局 14 辆参战车辆的 Tankopedia 总 HP 除以 14 得出，不再把整场平均值固定为 2400。`potential-DPB` 已接通 direct HP damage 事件链路和 `alphaDamage` 炮伤数据源，剩余核心缺口是回放实际进场血量与特殊伤害/真实样本校验。
...
- `average_hp`：本局 14 辆参战车辆的 Tankopedia 总 HP / 14；双方所有玩家使用同一均值。车辆库缺失 HP 时仅该单车暂定 2400。
- `common/tankopedia-tier{7,8,9,10}.json` 已生成 `alphaDamage`，来源为 BlitzKit `tanks.pb` 最高等级炮候选中的首发弹伤害。
...
- 回放实际进场 HP：当前均值使用车辆库基础 HP，尚未从回放确认每台车因配件、消耗品或模式产生的实际进场血量；后续若解析到该数据，可替换车辆库来源。
- 真实 `potential-DPB`：已从 Type 8 / subtype 8 / sub=3 direct HP damage 事件推断“击杀者 -> 被击杀者”的逐目标伤害和击穿次数，并结合 `alphaDamage` 填充潜在伤害补增；当事件缺失或无法映射时仍回退为 `potential_damage == damage_dealt`。
...
- `cd java && JAVA_HOME=D:\Env-Web-Java\jdk\temurin-21 mvn -s settings.xml -pl wotb-core test`
  - Core：4 tests，0 failures，0 errors，0 skipped。
- 真实样本：`D:\WpSystem\...\DAVAProject\replays` 中 6 场回放解析到 `killVictims=31`，`potential_damage_supplement=1004`。
...
1. 深挖 `battle_results.dat` 或 `data.wotreplay`，确认/解析每台车实际进场血量，并在可用时替换车辆库总血量来源。
