...
- [x] 战局回放炮线动画：命中（DAMAGE/KILL）与未命中开炮（SHOT，type 8/sub 8 非直接伤害结果）均已渲染。
- [ ] 战局回放**敌方**未命中炮线待 type-5 Spotting（点亮）逆向完成后再启用：当前 fail closed——
      位置流覆盖（type-10）≠点亮，不得冒充「被观测」（用户已确认维持现状，等待单独逆向任务）。
