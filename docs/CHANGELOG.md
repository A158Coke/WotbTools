...
- **战局回放总血量条开局满血回退**：`battlePlayback.vehicleHpAt` 无 ≤t 采样时，存活车辆（未阵亡或 t 早于 deathSec）回退 `maxHp`——回放整场覆盖（0s 起）、propId=3 仅在受击/血量变化时上报（首采样=首次变化），开局/未受击车辆血量=maxHp 为确定性事实；开局双方总血量条显示满血而非整条灰+「未知 N」。已阵亡且无采样保持 UNKNOWN 灰段（不冒充满血/0）。前端测试同步更新（满血回退 / 阵亡无采样灰段 / sentinel 忽略）。
