...
### Added
- **百场回放单文件上限 5MiB（域内独立）**：新增 `HundredBattleSubmissionService.HUNDRED_MAX_REPLAY_SIZE=5MiB`（全局 `ReplayUploadValidator.MAX_FILE_SIZE` 保持 20MiB 不动，名人堂 / AI 复盘 / 批处理 contract 不变）；超过 → 400 `HUNDRED_REPLAY_TOO_LARGE`；前端三语错误提示同步。
