...
- **Wargaming IdP client_id 收敛到既有 `WG_APPLICATION_ID`（单一凭据来源，不新增 secret）**：`infra/tofu/keycloak/identity-providers.tf` 原先给 `wargaming-asia` / `wargaming-eu` / `wargaming-na` 三个实例写入占位 `client_id = "not-used"`，与生产已有的 GitHub Secret `WG_APPLICATION_ID` 形成配置漂移（该 secret 本来就通过 Keycloak runtime env 供自定义 SPI 读取）。现在新增敏感变量 `var.wargaming_application_id`（`sensitive = true`、`nullable = false`，validation 拒绝空值与已知 placeholder：`not-used` / `bootstrap-not-configured` / `dummy` / `empty` / `juhe` / `juhe-qq` / `not-configured`），三个区服实例统一改为 `client_id = var.wargaming_application_id`（同一 Blitz application_id 跨区通用，仅一处变量引用）。Deploy 的 Keycloak OpenTofu apply step 复用**既有** `secrets.WG_APPLICATION_ID` 注入 `WG_APPLICATION_ID`（不引入 `TX_WG_APPLICATION_ID` / `WG_CLIENT_ID` / `WARGAMING_CLIENT_ID` 等重复凭据），`deploy/tx/keycloak-tofu.sh` 在缺失、空值或 placeholder 时 fail-closed 并导出 `TF_VAR_wargaming_application_id`（不打印值、不进 tfvars、不进 Tofu output）。Keycloak runtime 的 `WG_APPLICATION_ID` 注入逐字保留，因为自定义 SPI 仍通过 `System.getenv("WG_APPLICATION_ID")` 读取。`provider_id=wargaming`、alias、display name、region、回调路径、enabled 归属与 WG 认证协议均未改动；OIDC adapter 的 `client_secret_wo = "not-used"` 与 `https://unused.invalid` 保持不变。`scripts/ci/test-keycloak-tofu-contract.sh` 固定这条 WG 链路（step → `WG_APPLICATION_ID` secret、三个实例共用同一变量、变量敏感性与 placeholder 校验、禁止重复凭据、禁止 Tofu output），`deploy/test-keycloak-tofu.sh` 增加 WG 缺失/空值/placeholder 的 fail-closed 负例与真实 fresh-realm Admin API representation 断言（三个 Wargaming 实例的 `config.clientId` 等于 fixture，且第二次 plan 为 no-op，即 in-place 收敛、无 replacement）；QQ 的 `TX_QQ_CLIENT_ID` / `TX_QQ_CLIENT_SECRET` / `TX_QQ_CLIENT_SECRET_VERSION` → `TF_VAR_qq_*` → `keycloak_oidc_identity_provider.qq` 既有映射经审计后未变。
- **直接 GitHub Runner → TCR 发布 PoC benchmark（仅手工触发，不改生产发布链）**：TX workload 的现有发布链是 Build → GHCR → OCI archive → rsync 到 TX → `docker load` → TX 侧 `docker push` 到 TCR；run `35670563899` 三次 900s rsync 全部超时（`ERROR: rsync transport failure persisted after 3 attempts`），而 TX → GHCR 反向拉取在 171.1 MB 层上只取到 44.97 MB、56m16.97s 后 `connection reset by peer`，因此 OCI 传输与 GHCR 都不是 TX workload 的可靠镜像通道。新增 `.github/workflows/benchmark-tcr.yml`（**仅 `workflow_dispatch`**：无 push/pull_request/workflow_run，因此永不接力 Deploy、不产出 `deployment-manifest`、不动生产 Compose），用 `docker/setup-buildx-action` + `docker/build-push-action`（`load: true`，本地 Docker exporter）把**真实** `docker/Dockerfile.backend` **只构建一次**并 load 进 runner 的本地 Docker daemon，随后用**单独计时**的一次原始 `docker push` 把同一镜像发布到 `<vars.TCR_REGISTRY>/<vars.TCR_NAMESPACE>/wotbtools-backend:benchmark-<run id>`：不经 GHCR 发布、不做 OCI tar 传输、不做 rsync/scp/SSH、不在 TX 上 `docker load`、不 pull 回镜像、发布阶段绝不重新构建，也不调用 `transfer-oci-to-tx.sh` 或 `publish-loaded-image-to-tcr.sh`；**刻意不使用** BuildKit registry exporter（`type=image,push=true`），因为它在构建过程中就并发上传 blob，会把 BUILD 与 TCR UPLOAD 两个耗时合成一个数字，而这两个数字必须分开观测；标签既不是 `latest` 也不是 `sha-<12>`（生产 Deploy 只解析这两个），因此不构成任何 release identity，也不削弱 immutable image 契约。凭据沿用 Deploy 的仓库级 `TCR_USERNAME`/`TCR_PASSWORD` 与 `vars.TCR_REGISTRY`（`deploy.yml` 的 `image_existence` job 在无 `environment` 的 job 内已证明其仓库作用域），**不挂 `environment: tx-production`**——该 environment 无 protection rule，挂上只会给一次 benchmark 留下生产 deployment 记录。可观测性（三段独立计时、机器可读）：`stage=build` / `stage=tcr-push` / `stage=tcr-verify` 各自输出 `duration_seconds` 与 `result=PASS|FAIL`，另有 `stage=benchmark-identity`/`benchmark-start`/`build-start`/`benchmark-end`/`benchmark-total` 提供 identity 与总耗时；首跑**刻意无 cache、无任何重试包装**（不引入 `is_transient_*` 式重试），原始失败文本原样保留以便归类 DNS/TCP/TLS/auth/registry API/blob upload/manifest/timeout/connection reset/rate limit。发布验证只用 `docker buildx imagetools inspect --format '{{.Manifest.Digest}}'` 做 manifest 读回（绝不把镜像 pull 回来），要求读回成功且返回合法的 manifest descriptor（`sha256:<64 hex>`），成功时输出 `TCR_TARGET=`/`TCR_TAG=`/`TCR_DIGEST=`/`TCR_PUBLICATION=PASS`（不含任何凭据）；读回失败或形状非法被显式标注为 publication/identity 结果而非网络结果。（`load` 之后 runner 本地镜像与 TCR registry manifest 本就属于两个不同对象，因此本设计不再做 build digest == 读回 digest 的 parity 门，只对 registry 侧 descriptor 做可读性与形状门禁。）`scripts/ci/test-workflow-contract.sh` 新增该 benchmark 的安全契约：trigger 恰好是 `workflow_dispatch`、`permissions` 恰好 `contents: read`、不得挂 `environment`、`uses` 恰好是 checkout/TCR login/setup-buildx/build-push、step 文本不得出现 `ghcr`/`transfer-oci-to-tx.sh`/`publish-loaded-image-to-tcr.sh`/`rsync`/`scp`/`ssh`/`docker load`/`type=oci`/`upload-artifact`/`deployment-manifest`/`crane`/`latest`/`retry`/`continue-on-error`、tag 必须来自 `benchmark-$GITHUB_RUN_ID` 且以当前 main HEAD 为前提，并继续断言 GHCR+OCI+TX 发布链与两个 TX helper 仍然存在；另有断言固定「build 步骤必须 `load: true` 且**不得**带 `outputs`（即不允许 registry exporter）」「任何 step 都不得再出现 `docker build`/`buildx build`（发布阶段绝不重建）」「push 步骤必须是一次原始 `docker push \"$TARGET_IMAGE\"`，且不得 `tee`/`2>&1`/重定向/`timeout` 包裹」「`stage=build`、`stage=tcr-push`、`stage=tcr-verify` 三段 `duration_seconds` 必须都存在」。验证：`bash scripts/ci/test-workflow-contract.sh`（含 12 个 mutation 负例：push trigger、GHCR login、`latest` tag、`sha-<12>` tag、`environment: tx-production`、retry action、deployment-manifest 上传、TX SSH setup、用 registry exporter（`outputs:`）取代本地 `load`、在 push 步骤里重新 `docker build`、把 `docker push` 管道给 `tee`、把 `docker push` 包进 `timeout`，全部被拒绝）、`deploy/test-build-workflow.sh`、`deploy/test-tx-publication-helper.sh`、`scripts/ci/test-transfer-oci-to-tx.sh`、`deploy/test-release-plan.sh`、`scripts/ci/test-path-filters.py` 全绿；本次**不执行 benchmark run**（首跑由 operator 手工触发），生产发布链、`transfer-oci-to-tx.sh`、`publish-loaded-image-to-tcr.sh` 与 GHCR 发布路径全部未改动。

- **分布式 Replay create 的 MinIO/S3 失败保留完整 cause chain（诊断性变更，不改业务语义）**：生产 `POST /api/replay/processing-jobs` 只返回 `PROCESSING_JOB_STORAGE_UNAVAILABLE` / 409，日志里只留下 `event=replay_processing_input_discard_failed`——真正的输入 put 失败原因（`IOException → ErrorResponseException/MinioException → S3 error message`）在 `catch (IOException)` 里先被 `discardInputs()` 的记录挤掉、而两者都只打印 `getMessage()`，因此底层存储错误在生产上不可见。`ReplayProcessingJobService.createAndSubmit()` 现在在回滚**之前**记录 `event=replay_processing_input_persist_failed jobId=… error=…`，`discardInputs()` 的 `event=replay_processing_input_discard_failed` 一并改为 SLF4J Throwable overload：两个事件都带完整 Throwable，`io.minio` 的异常类型、S3 error message 与栈帧随 cause chain 落到日志（此前这些信息完全不存在）；日志不出现 access key / secret key / Authorization / signed URL / 签名（凭据只以签名形式出现，且 MinIO SDK 已把 `Signature=`/`Credential=` 替换为 `*REDACTED*`）。**已知渲染限制（本次刻意不越界处理）**：Spring Boot 的 `logging.structured.format.console=logstash` + logback `ThrowableProxyConverter` 对异常链渲染的是「类名 + `getMessage()`」，因此 `ErrorResponseException.toString()` 里额外的 error code/bucket/requestId 不会出现在渲染后的 `stack_trace` 中（它们随 Throwable 保留，但需要渲染端用 `toString()`）；若下一次生产测试仍需 code/requestId，最小后续是在 `MinioObjectStorage` 包装 `IOException` 时把 code 拼进 message，而本次 PR 严格不动该文件。**业务语义逐字不变**：HTTP 409、`PROCESSING_JOB_STORAGE_UNAVAILABLE`、cleanup 顺序与「回滚失败只记录、绝不替换原始 create 失败」的既有语义、MinIO 重试、RabbitMQ 行为、job state 全部不动；不新增 recovery/retry、不引入 logging abstraction 或依赖、不改 MinIO/OpenTofu/IAM/WireGuard/部署拓扑，也不改动 create 之外的 Replay 代码。`DistributedReplayProcessingPathsTest` 加强/新增：输入持久化失败仍返回 `PROCESSING_JOB_STORAGE_UNAVAILABLE`；生产形状（第 1 个输入已写、第 2 个 put 失败 + delete 被拒）仍不派发、不留下本地 job 目录且残留对象语义不变；persist/discard 两个诊断事件各恰好一次、均为 WARN、都带 Throwable 且原始 cause chain（含底层 S3 error message）原样保留。

- **TCR 不可变发布增加有界重试（仅诊断为瞬态的网络/registry 失败）**：run `35663436097` 证明 Docker build、GHCR immutable 发布与校验、OCI 导出、OCI 传输、archive SHA256、`docker load`、canonical loaded ref 全部健康，而 frontend 完成整条发布路径的同时 backend/keycloak 在 `Publish <component> loaded image to TCR` 以 `net/http: timeout awaiting response headers` 失败（TCR blob upload 已有多层 `Layer already exists`/`Pushed`）；这不是 PR #345 处理的 identity 问题。因此 `deploy/tx/publish-loaded-image-to-tcr.sh` 的 TCR immutable `docker push` 现在有**有界重试**：最多 3 次（`RETRY_MAX_ATTEMPTS`，上限硬校验 ≤3），首次失败后退避 5s、其后 15s（`RETRY_BACKOFF_FIRST_SECONDS`/`RETRY_BACKOFF_LATER_SECONDS`，测试可置 0）。只有被诊断为瞬态的失败才重试（`is_transient_tcr_failure`：`timeout awaiting response headers`、TLS/i-o/读写 timeout、connection reset/refused/timed out、broken pipe、unexpected EOF、network unreachable、temporary failure、429/5xx 与 `internal server error`/`bad gateway`/`service unavailable`/`gateway timeout` 文本）；空输出、未识别的诊断、认证/权限失败（`401 Unauthorized`/`denied`）、malformed request 与 timeout kill 一律**立即** fail-closed，不做无依据的重复。重试只是重新上传 TCR 尚未接受的 blob：由于 registry 是 content-addressed，已接受的 layer 会被复用，**绝不**触发 rebuild、GHCR repush、OCI 重传、`docker load`、release identity 重新打 tag、部署或 `latest` 发布。immutable tag 的读回（`docker buildx imagetools inspect`）跨越同一个 TCR 网络边界，因此带**同样有界**的独立重试（最多 3 次，独立 stage，不与 push 合并成一个状态机）；读回成功且 descriptor 形状合法（`sha256:<64 hex>`）才推进 `latest`，读回最终失败即 fail-closed 且 `latest` 保持不动。`latest` 仍是单次推送、不重试，顺序仍严格是 immutable push → immutable 读回成功 → 才更新 `latest`。本地确定性步骤（`docker image inspect`、canonical loaded-image 校验、`docker tag`、`docker load`、archive SHA256）一律不重试。日志按需求输出 `component=`/`stage=push-immutable`/`attempt=n/3`、失败尝试的 `result=RETRY` 与终止的 `result=PASS`/`result=FAIL`、`attempts=n`（不输出任何凭据），并保留既有 publication-start/publication-end 观测。`deploy/test-tx-publication-helper.sh` 新增用例：首次成功不重试、瞬态失败一次后第二次成功（含 immutable 校验与 `latest` 发布）、瞬态失败耗尽 3 次预算（无 `latest`、无校验、无 `result=PASS`）、永久拒绝与 timeout kill 不重试、读回瞬态失败第二次成功后仍发布 `latest`、读回持续失败 3 次后 fail-closed；`deploy/test-build-workflow.sh` 的 helper 契约断言改为校验重试接线（`run_tcr_stage push-immutable`/`run_tcr_stage verify-immutable`/`is_transient_tcr_failure`，且 `latest` 不在重试包裹内）。本 PR 不改 GHCR/TCR 命名、不改 tag 语义、不引入 `wotb-transfer` 或任何 image/config digest 比对。

- **TX OCI→TCR 发布链化简（release identity = immutable tag）**：Build→TX→TCR 的 release identity 现在只有一件事——不可变 tag `sha-<12>`；archive 的 SHA-256 是唯一的**字节**证明，两个**引用存在性**检查是唯一的**身份**证明。具体删除的机制：(a) `EXPECTED_IMAGE_ID` / `BUILD_IMAGE_ID`（Build 导出的同一次 build 的 image id）与 TX import 端的 `docker image inspect --format '{{.Id}}'` 读取、其 `sha256:<64 hex>` 形状校验和与期望值的相等比对；(b) `EXPECTED_DIGEST`（BuildKit manifest digest 经 `GITHUB_ENV` 传到 TX）与 `deploy/tx/publish-loaded-image-to-tcr.sh` 里「TCR manifest digest 必须等于该 build digest」的 parity 门。保留并成为唯一门禁：runner 与 TX 各自 `sha256sum` 该 OCI archive 并要求完全相等（不等即 fail-closed，不 promote、不 load）；`transfer-oci-to-tx.sh import` 在 import lock 内 `docker load -i` 后只证明 `<GHCR prefix>-<component>:<sha-12>`（由 helper 自己的 `GHCR_IMAGE_PREFIX` + 已校验 component/tag 派生，不再从 workflow 环境接收任何 identity 值）在本地存在；`publish-loaded-image-to-tcr.sh` 参数从 4 个（component/tag/expected-digest/loaded-ref）收敛为 2 个（component/tag），自行派生 loaded 与 TCR immutable/`latest` 名，`docker push` TCR immutable 后只读取该 TCR immutable tag 的 manifest descriptor 证明它确实存在（读取失败或返回值非 `sha256:<64 hex>` 即 fail-closed，`latest` 绝不被推进）。runner 端 `BuildKit output digest == GHCR immutable manifest digest` 校验保留：它证明唯一一次 build 确实以该 immutable tag 落到 registry，与 TX 侧的存在性检查同源。rsync 传输类失败的有界重试、`docker load`/publication 不重试、精确路径 cleanup、`wotb-transfer/*` 不复存在等边界全部不变。`scripts/ci/test-transfer-oci-to-tx.sh` 的 fake TX docker 改为「引用存在性」模型（state 只记录已 load 的 ref，`image inspect` 只做存在性；`--format '{{.Id}}'` 与 `docker tag` 都成为契约违规），并新增「调用方环境里残留 `EXPECTED_IMAGE_REF`/`EXPECTED_IMAGE_ID` 也不改变被检查的引用」用例；`deploy/test-tx-publication-helper.sh` 用 `target-digest-fail` 与 `target-digest-invalid` 两个模式证明存在性读取的 fail-closed，并断言多传一个 identity 参数会被拒绝；`deploy/test-build-workflow.sh` 与 `scripts/ci/test-workflow-contract.sh` 断言三个 TX job 不再出现 `EXPECTED_IMAGE_ID`/`EXPECTED_DIGEST`/`{{.Id}}`/`expected_digest`、publish 调用只带 immutable tag，且两个 helper 的 `GHCR_IMAGE_PREFIX` 默认值与 workflow 的 `GHCR_IMAGE_PREFIX` 保持一致。
- **Caddy-only TX 部署不再要求无关应用镜像的 immutable identity（并禁止 identity drift）**：`deploy/tx/deploy.sh` 原先在 `stage_and_validate` 中无条件为 wotb-frontend/keycloak/business-api 解析 immutable identity（`current_or_target_tag`），因此 `WOTB_DEPLOY_SERVICES=caddy` + `WOTB_DEPLOY_IMAGE_SERVICES=` 会在触碰 Caddy 之前以 `current immutable image identity is unavailable for wotb-frontend` 中止（run `35648585435`：生产 `tx-production-release.json` 只有 business-api/keycloak）。现在 identity 需求由「本次部署是否真的**启动**该应用镜像服务」决定（`starts_image_service`，`apply_services` 复用同一谓词）：Caddy-only 与既有数据库/RabbitMQ 类选择器一致，只拉取并仅重建 Caddy，并且
  - 本次**不启动**的服务按**实际生效**的 identity 渲染，来源顺序为 release metadata → live Compose document（逐字读取，兼容仍记录 GHCR 等旧引用的主机）→ 该 Compose service 的**运行中容器**（`docker compose ps -q <service>` + `docker inspect {{.Config.Image}}`，只针对该服务，不搜索本地镜像），且只接受 `sha-<12>` immutable tag；
  - 绝不被赋予本次 release 的 `TAG`（否则会把从未部署的服务的 identity 前进到新版本，造成配置/运行时漂移）；当该 host 持有该服务的部署状态（metadata 记录、live Compose 的具体引用或运行中容器）而三个来源都给不出合法 immutable identity 时 **fail-closed**（明确报错并指名服务，不 promote、不启动任何容器、不写 metadata）；仅当该服务在本机**从未部署过**（PostgreSQL/broker bootstrap：metadata、live Compose 具体引用与容器都不存在）时才保留 incoming 声明——此时既没有可保留的 identity，也没有被本次启动，因此不构成 drift；
  - 真正启动应用镜像服务的选择器（`all`/`keycloak`/`wotb-frontend`/`business-api`）仍 fail-closed：metadata 与 live Compose/运行容器都无法给出 immutable identity 时，必须在启动任何容器之前失败。
  此外 Caddy-only 不再重建 frontend 容器——重建 Caddy 后改用 `docker compose exec -T wotb-frontend nginx -t` + `nginx -s reload` 在**运行中的**容器内就地重新解析 `set_real_ip_from caddy`（保留 nginx 按真实客户端 IP 限流的边界，同时不拉取、不重建任何应用镜像）。`deploy/test-tx-runtime-config.sh` 覆盖生产回归（live frontend = `sha-dc0ff4554335`、本次 `TAG` 为另一个值、selector=caddy：frontend 不被重建、promoted Compose 仍是 `sha-dc0ff4554335`、metadata 不出现 frontend、Caddy 重建成功），并覆盖「live Compose 只有可变引用 → 用运行中容器 identity」「两处都无法推导 → 保留 incoming 声明且绝不回退到本次 TAG」以及同一状态下 `wotb-frontend` 部署仍失败且不启动任何容器。
- **TX OCI 传输 identity 契约修正（单一 canonical artifact identity + digest）**：Build 的 OCI exporter 由 image-push tags 命名、`name=` 在 `--tag` 存在时被忽略，因此 archive 里不存在 `wotb-transfer/<component>:sha-<12>`（run `35646876773` 的 frontend/keycloak `docker load` 结果只有 `ghcr.io/a158coke/wotbtools-*:sha-<12>` 与 `:latest`），而 `scripts/ci/transfer-oci-to-tx.sh` 的 import 仍按旧契约断言该 tag 存在，于是 `loaded-tag-verify` 失败、TCR publication 被跳过。审查后确认 `wotb-transfer/*` 相对 immutable `sha-*` tag、image digest、OCI archive SHA256、run-id staging 与 import/publication lock 不提供任何独有的安全、来源、碰撞隔离或并发属性，只会多出一条易漂移的命名契约，因此该中间 image namespace 被删除：验证 authoritative digest 的那一步把 `EXPECTED_IMAGE_REF`（`<GHCR prefix>-<component>:sha-<12>`）与 `EXPECTED_IMAGE_ID`（同一次 build 的 image id）导出给 TX；import 在 import lock 内先 `docker image inspect` 证明 archive 确实加载了该 reference 且 image digest 等于 `EXPECTED_IMAGE_ID`（缺失、非法 id 或 mismatch 一律 fail-closed，绝不进入 publication），`deploy/tx/publish-loaded-image-to-tcr.sh` 改为只接受该已加载 release identity 并直接派生 TCR immutable/`latest` 名，TCR immutable push → 仅 TCR manifest digest parity → `latest` 仍是最终 fail-closed 门；分发仍只有一次 build，TX 仍不拉取任何 registry。`scripts/ci/test-transfer-oci-to-tx.sh` 用有状态 fake TX docker 复现旧 mismatch 并覆盖：canonical identity + image digest 绑定通过、archive 未带 canonical identity、image digest mismatch、`docker tag`（已删除的 TX-local 别名）为契约违规、`EXPECTED_IMAGE_REF`/`EXPECTED_IMAGE_ID` 缺失、`latest`、大写、指向 TCR-local 名或注入一律 fail-closed；`deploy/test-tx-publication-helper.sh` 覆盖已加载 release identity 直接打 TCR 名、TCR digest parity、以及 ref/digest/component 校验与无 pull/crane；`deploy/test-build-workflow.sh` 与 `scripts/ci/test-workflow-contract.sh` 断言三个 TX component 都不再出现 `wotb-transfer`、导出不再声称 TX-local identity、且两个期望值绑定各自的 immutable GHCR tag 与 build image id。

- **TX application images via resumable OCI archive transfer**：backend、frontend、keycloak 各只构建一次，同时发布 GHCR immutable `sha-<12>` artifact 与 `latest`，并导出同一 BuildKit OCI archive。runner 仅在 BuildKit digest 等于 GHCR immutable manifest digest 后，用 `scripts/ci/transfer-oci-to-tx.sh` 经 job-scoped native OpenSSH 上的 resumable rsync 把 archive 传到 TX 的 job-scoped staging 路径 `/opt/wotb-tx/replication.incoming/<run-id>/<component>/<component>-<tag>.oci.tar.part`——该路径只由 component allowlist、`sha-<12>` tag 与纯数字 run id 在 helper 内部构造，workflow 不传递任何远端路径；`--partial --append-verify` 让断线从保留的 partial 文件续传而不是从 byte zero 重传（archive 因 Build 的 `oci-mediatypes=true` 已是 gzip layer，故刻意不再叠加 `--compress`）。只有 rsync 上传本身做有界重试（默认 3 次；仅重试确定的传输类 rsync 退出码，或 255/23/24 且诊断文本确为传输失败；`Permission denied`、`Host key verification failed`、认证失败、路径/选项错误与本地校验失败一律立即失败），`mkdir`、SHA256 校验、`docker load`、digest parity 与 TCR publication 都不重试。runner 与 TX 各自 `sha256sum` 并要求完全相等后，才把 partial promote 为完整 archive，`docker load -i` 只读取该已验证 archive（单一 `oci-transfer.lock` 串行，避免三个并发 multi-GB import 压垮 TX），load 后必须验证 `wotb-transfer/<component>:sha-<12>` 存在，最后由独立步骤运行既有的 `deploy/tx/publish-loaded-image-to-tcr.sh` 完成 TCR immutable push → 仅 TCR manifest digest parity → latest，并在成功后只按精确路径清理该 archive。publication 不包在 generic network retry 内，因此 TCR auth/publication 失败或 digest mismatch 都不可能触发 archive 重传；任一传输、校验、load、push、lookup、mismatch 或 latest 失败仍阻止 deployment manifest；TX 不拉取 GHCR，不使用 `crane` 或 `appleboy/scp-action`，也不把 GHCR/TCR credentials 经 SSH 传递；Yecao parser-worker 与 MinIO 保持 GHCR-only，DNS 与运行时服务拓扑未改变。历史记录：single long-lived SSH OCI stream 在 run `35630934506` 的 Keycloak import 以 SSH exit 255 失败（且 255 同时可能来自认证、主机密钥或配置错误，不能整体判为可重试），因此被上述可恢复、可独立验证的传输路径取代；更早的 GitHub runner→TCR 大 blob upload（run `35603682885`）与 TX→GHCR blob pull（run `35615777225`）超时路径同样保持退役。
...
- **TX Caddy 升为生产公网入口（移除 Stage-I loopback 限制）**：`deploy/tx/docker-compose.yml` 的 Caddy 默认绑定由 `127.0.0.1` 改为 `0.0.0.0`——`${CADDY_HTTP_BIND:-0.0.0.0}:80:80`、`${CADDY_HTTPS_BIND:-0.0.0.0}:443:443`、`${CADDY_HTTPS_BIND:-0.0.0.0}:443:443/udp`（HTTP/3），operator 仍可用同名环境变量覆盖绑定地址。Caddy 成为栈内**唯一**允许绑定公网接口的服务：`keycloak-postgres`（`127.0.0.1:15432`）、`business-postgres`（`127.0.0.1:25432`）、Keycloak admin（`127.0.0.1:18080`）、RabbitMQ management（`127.0.0.1:15672`）继续只绑 loopback，RabbitMQ AMQP 只绑 WireGuard `10.20.0.1:5672`，`business-api` 与 `wotb-frontend` 不发布任何端口。`deploy/test-tx-runtime-config.sh` 删除 Stage-I loopback 断言，改为生产契约：源码级断言三个 `0.0.0.0` 默认绑定且旧 loopback 默认不得回归；渲染后的 `docker compose config` 断言恰好 3 个 `host_ip: 0.0.0.0`（80/tcp、443/tcp、443/udp）与 4 个 `host_ip: 127.0.0.1` + 1 个 `host_ip: 10.20.0.1`，并用 Compose 块解析证明只有 `caddy` 块可以出现公网绑定。仓库仍不做任何 DNS 变更（DNS cutover 仍是 operator 的受控外部操作），Yecao 回滚环境保持不变。

- **TX RabbitMQ provider mirror 自举（retry-safe staged bootstrap）**：`services=rabbitmq` 在 `tofu init` 前从 committed RabbitMQ lockfile 推导 `cyrilgdn/rabbitmq 1.10.1`，并把 `tofu providers mirror -platform=linux_amd64` **只写进 mirror 同文件系统的临时 staging 目录**；只有 staging 内校验来源、版本、`<version>.json` metadata、archive SHA256 与 lockfile `zh:` checksum 全部通过后，才把 provider 目录 rename promote 到 canonical `/opt/wotb-tx/tofu-provider-mirror/registry.opentofu.org/cyrilgdn/rabbitmq`。`tofu providers mirror` 不再直接写生产 provider 目录：中断或失败的下载只在 staging 留下部分字节并由 trap 清理，canonical mirror 保持原样、不写 marker、不进入 `tofu init`/`apply`。canonical 已有精确包且校验通过时幂等复用且不联网；exact-version 包损坏或 metadata 不完整时经一次 fresh staging bootstrap 自动修复（修复失败时 canonical 字节不变），不会永久 poison 后续部署；其他版本或多个 `linux_amd64` archive 仍 fail closed 且不静默删除。正式 init 继续只允许 filesystem mirror 且保留 second-plan no-changes 硬门禁。Keycloak/PostgreSQL provider 路径与 RabbitMQ topology ownership 不变。
- **全业务 E2E 门禁（`PRE_CUTOVER_READY` 代表真实业务链可用）**：只读门禁 `deploy/tx/pre-cutover-check.sh` 现除基础设施与路由 token 外，还用**专用机器身份** `wotbtools-e2e` 驱动真实业务链并逐项输出可判定 token：`auth-token`（client_credentials 取 token）、`tx-control-plane`（未知 job 必须 404 `JOB_NOT_FOUND`）、`anonymous-rejected`（匿名 401）、`admin-authz`（匿名 401 + 已认证非管理员 403）、`business-profile` / `business-hof` / `business-boost`、`hof-replay-storage`（真实 HoF 记录的 `GET /api/hof/{id}/replay` 可下载且非空）、`parser-worker`（`rabbitmqctl` 断言 `wotb.parser` 有 consumer、结果队列存在、DLQ 为空）、`processing-e2e`（上传已 staged 的回放 fixture → `202` → 轮询 READY，即 **TX backend → PostgreSQL 权威 → MinIO 输入 → RabbitMQ → Yecao worker → MinIO artifacts → result → READY**）、`dataset-result`（同一 job 的 `GET result`）、`map-overview` / `battle-playback-v2`（同一 job 的 dataset 读取；`204` 视为 artifact 缺失或不可用而失败）、`minio` / `ai-facts`（用 control_api 身份做 SigV4 预签名读取 `result/finalized.json` 与 worker 的 `artifacts/0/ai-facts.json`，**不做任何付费 AI 调用**）、`export`（创建 export job → 轮询 READY → 下载非空 artifact）、`business-data-integrity`（operator 提供的只读 Yecao 行数快照逐表比对 + `hall_of_fame_record` identity sequence ≥ max(id) 且 ≥ 355）。公网边缘按阶段拆开，而不是用 `-k` 伪装 TLS 就绪：切 DNS 前 `public-edge-sni-web` / `public-edge-sni-auth` 只证明 TX:443 可连通、TLS 握手完成且为该 host 出示证书（`curl` 退出码 `60`＝链尚未受信是**预期通过**状态），切 DNS 后 `pre-cutover-check.sh --post-cutover` 额外要求 `public-tls-web` / `public-tls-auth`（该 host 解析到 TX 地址、受信任证书校验通过、2xx），成功输出 `POST_CUTOVER_READY` / `DNS_CUTOVER_PERFORMED` / `WAITING_FOR_OPERATOR_RETIREMENT`；门禁全程**不关闭 TLS 校验、不使用** `-k/--insecure`。任一 token 失败即 `PRE_CUTOVER_NOT_READY`（post 阶段为 `POST_CUTOVER_NOT_READY`）；门禁对基础设施与用户数据保持只读，唯一写入是 30 分钟 TTL 自动回收的瞬时 processing job 与 export job。**同时移除 Caddy 的固定容器地址**：真实 TX 上 `172.29.0.2` 与动态分配的地址冲突（`Address already in use`），因此 Compose 不再 pin 任何容器地址，readiness surface 改绑 `http://caddy`（Docker service DNS），全部内部探针改用 `http://caddy/_wotb/...`，frontend nginx 的 `set_real_ip_from` 改为信任整个 `wotb_tx_internal` 子网（`172.29.0.0/16`）。新增 TX-internal 机器身份 `wotbtools-e2e`（confidential、仅 service account、无 redirect URI，由 OpenTofu 显式授予**唯一** realm role `wotbtools-user`；secret 走 write-only `client_secret_wo`，与 `wotbtools-admin-api` 同一模式），`infra/tofu/keycloak/validate-plan.sh` 把它纳入受保护资源，`deploy/test-keycloak-tofu.sh` 在真实 disposable realm 上断言它只有该角色。回放 fixture 由 Deploy workflow 从已提交的 `common/fixtures/replays` staged 到 `/opt/wotb-tx/e2e`（health-probe 只读挂载 `/e2e`），门禁因此能上传真实回放而不依赖 TX 上的仓库 checkout；SigV4 预签名 helper 在 CI 中用一次性 MinIO 容器做真实 PUT/GET 验证。parser DLQ 非空时的 operator 处置流程（只读确认权威状态 → 在 loopback Management UI 分类消息 → 修复后重新投递 → 记录永久失败样本 → 队列回到空后重跑门禁；**不允许**用 purge 充当修复）记录在 `docs/operations/rabbitmq.md`。
- **TX 路由终结与生产运行时收敛**：TX frontend 的 API upstream 由「经 WireGuard 指向 Yecao `10.20.0.2:8087`」改为 **TX-internal `http://business-api:8087`**；`TX_BACKEND_UPSTREAM` 的旧硬校验（必须等于 Yecao 地址）替换为「必须等于 TX 内部业务运行时地址」，并补齐正/负 fixture（接受 `http://business-api:8087`；拒绝已退役的 Yecao 地址、公网 host、错误端口、错误主机与 trailing slash）。`blocking_health` 与只读 `PRE_CUTOVER_READY` 门禁都**不再探测或依赖** Yecao backend 路径，改为对渲染后的 live Compose 断言两条不变量：`tx-internal-api-route`（frontend upstream 是 TX 内部业务运行时、`business-api` 不发布任何端口、任何服务都不得发布 8087）与 `distributed-execution-plane`（`business-api` 必须是 `WOTB_REPLAY_EXECUTION_MODE=distributed` + `WOTB_REPLAY_PROCESSING_JOB_REPOSITORY=jdbc`），任一失败即 `PRE_CUTOVER_NOT_READY`。deploy 侧新增 staging 阶段的 `assert_routing_boundary`：引用/发布已退役 8087 的 Compose、或重新启用本地执行面的配置，在 promote 之前就 fail-closed。Yecao 侧新增 `assert_parser_worker_execution_plane`：显式选中 parser-worker 时，staged Compose 不得给它数据库凭据、本地 replay job 目录或执行面开关——worker 必须保持无状态，否则拒绝 staging。`deploy/release_plan.py` 补上一直缺失的 `caddy` selector（target `tx`）并加 contract 断言，使 Caddy-only 变更可被影响面推导，而不是只存在于 deploy 脚本里。
- **TX 业务 API 运行时（`business-api`）与遗留容器去生产化**：TX Compose 新增 `business-api` = 现有 `ghcr.io/a158coke/wotbtools-backend` immutable 镜像的**单运行时**承载（全部 public business endpoint + 分布式回放控制面），**不发布任何 host port**，只被 TX-internal 的 frontend nginx、Caddy readiness surface 与 deployment-owned `health-probe` 访问（应用 `http://business-api:8087/api/health` 与 Spring dedicated management `http://business-api:8088/actuator/health`，两者都只接受 2xx）。它以 `WOTB_REPLAY_EXECUTION_MODE=distributed` + `WOTB_REPLAY_PROCESSING_JOB_REPOSITORY=jdbc` 显式关掉本地执行面（无本地解析、无本地 job 权威、派发走 RabbitMQ、dataset 与 artifact 走 MinIO），并挂 `replay_data` 卷到 `HOF_REPLAY_DIR`——HoF 回放原件是永久内容寻址文件，与 MinIO 的 `temp/jobs/` dataset 工作区职责分离，不引入任何新的回放存储抽象。凭据按服务分组 fail-closed：应用库角色 `TX_BUSINESS_DB_*`、RabbitMQ `control-api` 口令、MinIO `control_api` key pair、`KEYCLOAK_ADMIN_CLIENT_SECRET`、`AI_API_KEY` 只在该服务被选中时强制，未选中时只注入 Compose 渲染用的占位值（RabbitMQ-only / business-postgres-only 部署仍不需要它们，且不重建任何应用镜像）。`KEYCLOAK_ISSUER_URI` 保持 public URL（Keycloak 的 `iss` 由 hostname 决定，JWKS 也随公开名指向当前生效的 Keycloak），`KEYCLOAK_ADMIN_SERVER_URL` 故意改为 TX-internal `http://keycloak:8080`：DNS cutover 前公网地址仍由 Yecao Keycloak 应答，业务运行时绝不通过公网管理那个 realm。TX deploy 侧补齐 image service（当前/目标 immutable tag 解析与重写）、metadata identity、`all` 服务集、image pull、blocking health、失败即停止该服务，以及只在健康通过后记录 identity。Release plan 把 backend 镜像路由到 `business-api`（target `tx`）；Yecao legacy `wotb-backend` 仍保留为**显式** rollback/退役选择，但不再被任何推断选中（Yecao Compose 配置变更因此不再刷新它）。TX 拓扑/Compose 变更现在从冻结 commit 重建 backend+frontend+keycloak 三个应用镜像并发布同一组 TX 服务，保证「只改 Compose」的变更真实到达运行中的业务运行时，而不是只改文件。只读 `PRE_CUTOVER_READY` 门禁同步要求业务运行时的必需输入齐全，避免渲染期失败被读成 ready。
- **分布式 Dataset 消费（单运行时内）**：把分布式链路里「per-source battle 在读取侧临时拼成 dataset」替换成与本地完全相同的批次收尾链——`parser.result` → sources terminal → **FINALIZING_BATCH** → dedupe / 冲突判定 / League Rating / Rating V2 / 批次聚合 / enrichment → **finalized batch dataset 写 MinIO** → READY。新增 `ReplayBatchFinalizer`（`wotb-replay-coordinator`）作为批次语义的**唯一实现**，本地 `ReplayProcessingJobService.finalizeJob` 与分布式 `PostgresParserOutcomeHandler` 都调用它（finalize 进度三元组适配也统一，两条链路的 `processed/duplicates/failures` 语义一致）。关键前提：`Replays.ParsedEntry` 只携带 `(sourceIndex, sourceName, Battle, failureMessage)`，因此控制面仅凭「PG 里的 source 终态 + 对象存储里的 per-source canonical Battle」即可构造出与本地进程内**逐字相同**的收尾输入，**不需要改动 worker 实现或 wire 契约**。`ObjectStorageReplayProcessingResultReader`（读取侧拼接 per-source）被 `ObjectStorageReplayDatasetRepository` 取代，成为 `result/` 对象布局的唯一 owner：`result/source-<i>.json` 是收尾的**输入**，`result/finalized.json` 是读取侧**唯一**的数据集（`FinalizedDataset` 是带 `schemaVersion` 的显式对象契约镜像，不让 `isLeague`/`validCount` 之类派生 getter 参与序列化）。失败语义：去重/聚合后 0 场有效 → 终态 `NO_VALID_REPLAYS`；READY 的 source 缺 per-source 输入 → 收尾 fail closed（`IOException` → 投递不 settle → DLQ + operator 重放，绝不把数据缺失算成「空批次」）；收尾期间的存储故障同样不写成 job 终态；收尾期间被取消 → 按取消语义收尾；READY 仍**不带内存 dataset**（**PG = lifecycle 权威，MinIO = dataset 与 artifact 权威，distributed 生产不依赖 TX 本地磁盘**）。读取边界收口为单一端口 `ReplayProcessingResultReader`（新增 `aiFacts`/`mapOverview`/`battlePlaybackV2` 字节方法与 `local` 实现 `LocalReplayDatasetRepository`），因此 `GET .../result`、Export、Rating V2、AI Review（`ai-facts.json`）、Map Overview、Battle Playback V2 全部经它取数据，字节 → DTO 解码由 `ReplayArtifactWriter.decode*(...)` 唯一拥有；`acquireForExport` 不再要求 `job.result() != null`（distributed 下进程内本就不持有 dataset），只要求状态 READY。TTL 清理改为 **lease-aware**：`ReplayJobAuthority.deleteExpiredTerminal`（集合式 delete，看不见进程内 Dataset Lease）替换为 `listExpiredTerminal` 候选集，由 `ReplayProcessingJobStore` 按 lease 过滤后逐条删除，与本地 sweep 同一判定——否则正在被 AI / Export 读取的 job 行会被删掉、读取中途变成 404。**可读获取不再依赖 JVM 状态**：`acquireForSource` / `acquireForExport` 走 `get(jobId)` 的可读语义（live registry 命中，否则从 PostgreSQL 恢复只读投影且**不**写回 live registry），因此 TX backend 重启后 `GET result` / AI Review / Map Overview / Battle Playback V2 / Export 依然可用（只需 PG 有 job/source 状态、MinIO 有 dataset/artifact）；原先只读 live `jobs` map 的实现会让「重启后读取能力全挂」。**过期回收补齐对象存储工作区，顺序为先 MinIO、后 PostgreSQL**：新增 `ReplayJobWorkspaceCleaner` 端口（job 级：实现按 `ObjectStorageKeys` 从 `jobId` + sources 推导 `input/<i>/<name>`、`result/source-<i>.json`、`result/finalized.json`、`artifacts/<i>/*` 的确定键集合，**不**给应用层任何 `list`/任意前缀删除能力），由 `ObjectStorageReplayJobWorkspaceCleaner` 实现；权威侧 sweep 对每个 lease-free 的过期终态 job 先清工作区、成功后才删权威行，任一失败都只记录并保留权威行、下一轮幂等重试（「MinIO 删成功 + PG 删失败」重复回收无害；顺序反了会在 MinIO 失败时留下再也没人知道该删的孤儿对象）。桶上 `temp/jobs/*` 的 1 天 lifecycle 明确降级为**孤儿兜底**，不是正常回收机制；不新增任何按天清理的 workflow。
- **TX Replay Control Plane（单运行时内）**：`POST/GET/DELETE/GET result /api/replay/processing-jobs` 现由分布式实现承载，HTTP 契约（路径/方法/202/204/404 `JOB_NOT_FOUND`/409 `JOB_NOT_READY`/503 `PROCESSING_QUEUE_FULL`/`TOO_MANY_REPLAY_FILES`/`SOURCE_NOT_FOUND` 与 `ProcessingJobResponse` 字段集）逐字不变。新增**单一正向枚举**开关 `wotb.replay.execution.mode`（env `WOTB_REPLAY_EXECUTION_MODE`）= `local`（缺省，行为与引入前逐字一致）| `distributed`，**未知值（含显式空值）启动即失败**——校验放在总是存在的 `ReplayProcessingConfig` 构造器里，因为 `@ConditionalOnProperty` 无法表达「值非法」，未知值只会让两侧都不匹配而静默降级为 local。装配点 `com.wotb.web.replay.config.ReplayDistributedConfig` 整体按属性门控（local 下**不创建任何 bean**），local 执行组件（`ReplayParseScheduler` / `LocalReplayProcessingDispatcher` / `LocalReplayProcessingExecutor`）反向按 `havingValue=local, matchIfMissing=true` 门控，因此两种模式的执行平面永远只有一套、distributed 下 TX **不存在任何本地解析路径与 worker 线程池**。create 编排复用唯一的 `ReplayProcessingJobService`，只把「输入落点」与「dataset 读取」抽成两个端口：`ReplayProcessingInputStore`（distributed 实现写 MinIO `temp/jobs/<jobId>/input/<i>/<name>`，键只由 `ObjectStorageKeys` 生成）与 `ReplayProcessingResultReader`（distributed 实现读 `temp/jobs/<jobId>/result/source-<i>.json` canonical per-source dataset，读不到沿用 409 `JOB_NOT_READY`，不发明新错误码）；派发是**确认式** AMQP 投递，`submit` 抛异常即 create 失败并回收已登记的 job 与输入，绝不留下没有执行绑定、永远不会有结果的 job。结果消费 `wotb.parser.result` → `ParserResultListener`（manual ack，幂等 no-op 同样 ack、不进 DLQ）→ `PostgresParserOutcomeHandler`：判定顺序「未知 job → 陈旧 attempt → 已终态 → 已取消 → 逐 source 应用」，重复/陈旧一律 `IGNORED`，状态迁移仍由 `ReplayProcessingJob`/`ReplayJobState` 唯一拥有（**不引入第二套状态机**）；陈旧判定依据新增 Flyway `V24` 的 `replay_processing_job.attempt_watermark`（单调，默认 0，`ReplayJobAuthority` 提供读写；控制面重派 `attempt+1` 时同样推进它，因此同一 attempt 的重复失败报告自然成为陈旧 no-op）。**逻辑重试由控制面唯一拥有**（PR D 已把重试权从 worker/broker 收回）：worker 的基础设施失败只发布确认投递的 `parser.failed(retryable=true)`，控制面消费 `wotb.parser.result` 上的两种报告后按同一判序处理——未知 job / 陈旧 attempt / 已终态 / 已取消一律 `IGNORED` 并 ack 丢弃；`retryable && attempt < wotb.replay.retry.max-attempts`（env `REPLAY_PARSER_MAX_ATTEMPTS`，缺省 3）时**立即重派**同 `jobId` 的 `attempt+1`（attempt 由控制面写入 `ReplayProcessingRequest` 并持久化在 PG 水位线里，broker 适配器只做 1:1 映射、不再硬编码 `attempt=1`），预算用尽或 `retryable=false` 才逐源 FAILED + 终态；重派顺序是**先派发、再推进水位线**——派发失败即上抛（`ParserResultListener` nack 不重入队 → `wotb.parser.dlq` 等 operator 重放），水位线保持不动，因此重放能重新派发这一次重试，job 不会卡在「已决定重试却没有任何执行」；崩溃窗口（已派发、未推进水位线）最坏结果是同一次重试被执行两次，产物按对象键幂等覆盖、报告按同一 attempt 幂等应用，属重复工作而非不一致状态。重试**不经过** `wotb.parser.retry`（把 broker 队列当成控制面的重试调度器会让 broker 成为第二个重试计时 owner），也没有延迟队列。`wotb-web` 只新增对 `wotb-object-storage-minio` / `wotb-broker-rabbitmq` 的依赖，`wotb-replay-processing` 新增 `spring-boot-autoconfigure`（仅为执行面门控），不动任何 modules 列表、不新增 `wotb-control` 生产容器、不改前端/Android/Flyway 历史。**create 失败必须回滚对象存储输入（不靠桶生命周期兜底）**：`ObjectStorage` 端口由 PR B 的三方法扩展为四方法（新增 `delete(ObjectKey)`——**键级**、删除不存在的对象即成功、仍然**没有** `list`/前缀操作，因此调用方只能删自己用 `ObjectStorageKeys` 造出的键），`ReplayProcessingInputStore` 相应扩展出 `discard(jobId, files)`（与 `store` 同一套键推导，只落在 `temp/jobs/<jobId>/input/` 之下）；create 在「派发成功」之前失败的四条路径——输入半途写失败、权威登记失败、队列满、派发失败（确认失败/不可路由/超时）——都会删除已写入的输入并回收已登记的 job 状态，**回滚本身失败只记录日志、绝不替换原始 create 失败**（否则一次删除权限抖动会把「派发失败」改写成「存储失败」）。为了让这次删除在生产真的可用，`infra/tofu/minio` 为 `control_api` 增加**独立**的 delete-only 策略 `wotbtools-temp-control-api-reclaim`（仅 `s3:DeleteObject`，resource 仍是 `temp/jobs/*`；`worker` 依旧没有任何删除权限）并附上新 attachment——独立成第三份文档而不是给已 apply 的读写策略加一条 action，是因为后者在 plan 里表现为 in-place update，会被 `validate-plan.sh` 按设计拒绝；`validate-plan.sh` allowlist、`test-validate-plan.sh` fixture（reclaim 新增 PASS、delete/改动作 REJECT、second plan no-op PASS）与 CI MinIO state smoke 的「每个策略动作集与前缀精确匹配」断言同步扩展。`docker/Dockerfile.backend` 补上 `wotb-object-storage-minio` / `wotb-broker-rabbitmq` 的 `src` 拷贝（`wotb-web` 新增依赖把它们拉进了 `-pl wotb-core,wotb-web -am` 反应堆，只拷 pom 会让镜像在 `package` 时以 "package com.wotb.broker.rabbitmq does not exist" 失败），并把原本只覆盖 parser-worker 的「Maven 反应堆 ↔ Dockerfile COPY 闭包」断言**泛化到每一个跑 `-pl … -am` 的 Dockerfile**（闭包由各 pom 的 `com.wotb` 依赖推导，缺 src 即以文件名+模块名失败）。
- **Yecao Parser Worker 执行面**：新增可部署 Maven 模块 `java/wotb-parser-worker`（入口 `ParserWorkerApplication`，`web-application-type=none`）作为 **Yecao 唯一回放解析执行面**——只依赖 contracts/core/result/playback/replay-processing/object-storage-minio/broker-rabbitmq，**不依赖 `wotb-web`、不依赖 wotb-ai、没有任何数据库或 JPA 依赖**（worker 无 DB 凭据，job 权威状态仍只在 TX 的 PostgreSQL），启动时不声明任何 broker topology（唯一 owner 仍是 `infra/tofu/rabbitmq`）。解析**复用 canonical 路径、不 fork**：把本地执行器的执行体抽成与存储无关的 `ReplayProcessingSourceRunner`（canonical `DefaultReplayProcessingFacade` + `MapOverviewBuilder`/`BattleTimelineBuilder`/`BattlePlaybackProjector` + `ReplayArtifactWriter` 字节生成，错误分类逐字沿用 `PROCESSING_JOB_STORAGE_UNAVAILABLE`/`REPLAY_PROCESSING_FAILED`），`LocalReplayProcessingExecutor` 变成薄适配（job 目录布局、取消登记表、Micrometer 采集），artifact 落地经 `ReplayArtifactSink` 抽象——本地是 `ReplayArtifactFileSink`（`<root>/<jobId>/derived/r<index>/…`，临时文件 + atomic move），worker 是 MinIO sink（`temp/jobs/<jobId>/artifacts/<index>/…`），因此**不存在第二个 parser、也不存在第二套 artifact 生成逻辑**；worker 并发只有一个事实源：**consumer 并发**（`PARSER_WORKER_CONCURRENCY`，默认 2，对齐 `REPLAY_PARSE_MAX_CONCURRENT`；容器上 `concurrentConsumers = maxConcurrentConsumers = concurrency`），`PARSER_WORKER_PREFETCH`（默认 1）只是**每 consumer** 的未确认上限——单 consumer 加大 prefetch 不增加并行度，只是拉长崩溃后的重投窗口；worker 不引入第二套调度器。传输语义在 `ParserRequestListener`（manual ack，`prefetch` + `default-requeue-rejected=false`）：**只有 outcome 经 correlated publisher confirms 确认后才 `basicAck`**（`RabbitParserOutcomePublisher` 的构造守卫要求 `CachingConnectionFactory` 具备 correlated confirms + publisher returns 并强制 `mandatory`，否则拒绝装配）；基础设施工作失败（MinIO 读失败、**或解析成功但 artifact 写失败**）→ 发布确认投递的 `parser.failed`（同 `jobId`、同 `attempt`、`retryable=true`）→ **确认后才 `basicAck` 原请求**；**artifact 写失败必须按基础设施处理**：canonical runner 会把 sink 的 `IOException` 报成逐源 `PROCESSING_JOB_STORAGE_UNAVAILABLE`（本地 job 目录语义，保持不变），worker 侧 `ParserRequestHandler` 把这一分类还原为基础设施失败（`ParserArtifactStorageException`），绝不落成终态逐源 `parser.result FAILED`（否则一次瞬时存储抖动会被记成"回放永远不会解析"）；**outcome 投递不确定绝不转成第二个结果**：`parser.result` 的 confirm 丢失/超时/NACK/不可路由**不能证明**该结果未被 broker 路由，因此 worker 此时**不发布第二个 outcome、不 ack、不 nack**（原先把 `ParserOutcomePublishException` 当泛化基础设施失败并追发 `parser.failed(retryable=true)` + ack 的路径已删除），原请求保持未 settle，由 AMQP 重投**同一 attempt** 并重放出同一个 `parser.result`（`(jobId, attempt)` 幂等由 PR E 负责）；`PARSER_OUTCOME_NOT_DELIVERED` 这一 wire code 已整体移除，基础设施工作失败（MinIO 读/写）仍统一报 `PARSER_WORKER_STORAGE_UNAVAILABLE`。**重试策略归控制面（架构决定）**——RabbitMQ 不得自动重放 parser 工作：worker **不再 `basicNack(requeue=false)`**、不主动把任务送进 `wotb.parser.retry`，而是「发确认投递的 `parser.failed`（同 `jobId`、同 `attempt`、`retryable=true`）→ 确认后才 ack 原 `parser.request`」；若 `parser.failed` 本身投递失败则**既不 ack 也不 nack**，交给 AMQP 连接/信道重投同一个 attempt（绝不静默确认）。**逻辑重试 = 控制面按 PostgreSQL 权威状态决定 `attempt+1` 并派发新的 `parser.request`**（job 是否仍活跃 / attempt 是否仍是 N / 取消或过期 / 预算是否剩余），**传输重投 = 仅因原投递未被 ack 而再次投递同一 attempt**——两者不得混同。worker 不持有 retry 计数或预算、无 PG/JPA；`wotb.parser.retry` 拓扑按兼容性保留（worker 不使用），其移除属于 PR E 上线后的独立清理 PR。worker 不持有 attempt 计数、不引入 DB/JPA；业务失败（canonical 解析拒绝）→ 逐源 `parser.result` 记稳定 error code 并 `basicAck`（重跑同样的字节不会改变结果）；**无法解码的投递 → 原字节经 `parser.dead` park 到 `wotb.parser.dlq` 后 ack**——这是 worker 唯一能自主判定的终态（无可报告的 `jobId`/`attempt`，且重投同样的字节永不成功），避免 poison message 在重试环里空转；park 自身失败则不 ack（留在重试路径，不假装已处理）。`parser.result`/`parser.failed` 只带 PR C wire envelope 的既有元数据字段，artifact 与 canonical dataset（`result/source-<i>.json`，PR F 读取）都走 `ObjectStorageKeys.tempJobObject(...)` 推导键并幂等覆盖。新增 `docker/Dockerfile.parser-worker`（JDK 25 构建 → JRE 运行时，注入 `BUILD_COMMIT`，只构建 `-pl wotb-parser-worker -am` 反应堆）与镜像 `wotbtools-parser-worker`；`deploy/docker-compose.prod.yml` 新增无公网端口、无数据库凭据的服务 `parser-worker`（RabbitMQ 走 TX WireGuard `10.20.0.1:5672`/vhost `/wotbtools`、MinIO 走 `10.20.0.2:9000`，凭据复用 `TX_RABBITMQ_PARSER_WORKER_PASSWORD` 与 `YECAO_MINIO_WORKER_*` 两个既有身份名），`deploy/deploy.sh` 把它纳入服务白名单、镜像 tag 重写、metadata 记录与**容器存活 gate**（无 HTTP 端点，崩溃循环必须让部署失败并停止该服务，禁止 runaway restart），并且**只在显式选中 `parser-worker` 时 fail-closed 要求其三项凭据**——`all` 仍保持既有 legacy 服务集，因此未选中该服务的部署不需要新凭据、也不启动它（PR D 回滚语义：不选即不启动）。真实容器契约测试 `ParserWorkerPipelineTest` 在真实 RabbitMQ + 真实 MinIO 上跑完整链路：`parser.request` → worker → **同一份真实回放 fixture 同时经本地 sink 与对象存储 sink 处理并逐字节对拍**（4 个对象存在性与字节全等）→ canonical dataset → 确认投递的 `parser.result`；另有「解析拒绝 → 逐源 FAILED + ack」「基础设施工作失败 → 确认投递的 `parser.failed(retryable=true)` + ack（`wotb.parser.retry` 保持为空）」「`parser.result` 投递不确定 → 什么都不 settle（不追发第二个 outcome、不 ack、不 nack）」+「未 settle → 重投保持同一 attempt」「重投导致同 `(jobId, attempt)` 的重复 `parser.result` 被允许」「无法解码 → 原字节 park 到 DLQ + 不产生重试副本」等路径。`scripts/ci/test-workflow-contract.sh` 新增 **`java/pom.xml` modules ↔ Dockerfile COPY 闭包**断言（parser-worker 镜像必须预拷贝全量 pom，并按其 `-pl … -am` 依赖闭包拷贝 src），`deploy/test-deploy-contract.sh` 新增 parser-worker 的 compose/deploy/ephemeral-credential/存活 gate 契约。
- **RabbitMQ 全运行时双向协议**：新增 Maven 模块 `java/wotb-broker-rabbitmq`（`com.wotb.broker.rabbitmq`），把已存在的 `com.wotb.contracts.ReplayProcessingDispatcher` 端口与 parser 双向协议接到 AMQP 上，**不新建平行端口**：`RabbitReplayProcessingDispatcher.submit` 只发一条 `parser.request`（persistent、`attempt=1`），发布失败直接抛异常（fail-closed，不静默吞），`cancelQueued` 返回 `ACTIVE_COMPLETION_PENDING` 且**不发取消消息**——取消是 PostgreSQL 的 `cancel_requested` 状态变更，broker 侧不存在 cancel routing key，因此 RabbitMQ 不会成为第二状态权威；`ParserResultListener implements ChannelAwareMessageListener` 用 manual ack：handler 成功或判定陈旧/重复（`Outcome.APPLIED` / `IGNORED_STALE_OR_DUPLICATE`）都 `basicAck`，handler 抛异常或解码失败（非 JSON、非对象、未知 `schemaVersion`、未声明属性）一律 `basicNack(requeue=false)`，消息不丢、也不无限 requeue；端口 `ParserOutcomeHandler` 只表达「能不能应用」与陈旧判定，不承载传输语义。wire envelope 由 `ParserMessageCodec` 编解码，**只带元数据**（`schemaVersion` / `eventId` / `jobId` / `attempt` / 时间戳 / sources 的 `sourceIndex`+`sourceName` / 结果 `status`+`errorCode`），**不带对象键、URL、回放字节或解析内容**——worker 自己用 `ObjectStorageKeys.tempJobObject(...)` 推导键，键布局仍只有那一个事实源；未知或不兼容 `schemaVersion` fail closed。`schemaVersion` 常量由 `contracts/mq/parser-messages.json`（三个 envelope 的 JSON Schema，`additionalProperties:false`）与 `ParserMessageContractTest` 双向 pin 住，schema 不是死文件。`ParserTopology` 只提供 canonical 名字常量并写明「唯一 owner 是 `infra/tofu/rabbitmq`」，模块内**没有任何 Spring stereotype**（不引入会被 `wotb-web` 扫到的 `@Configuration`/`@Component`），接线留给 PR D/E；`RabbitBrokerProperties` 是不可变 record，构造器校验、`toString` 遮蔽密码。OpenTofu `infra/tofu/rabbitmq` 新增结果回流队列 `wotb.parser.result`（durable、classic、无 TTL、DLX→`wotb.jobs`/`parser.dead`、`prevent_destroy`）与两条 binding（`parser.result` 与 `parser.failed` 都指向它），`control-api` 的 read 由 `^$` 扩到 `^wotb\.parser(\.result)?$`（**不放宽** configure/write、不加任何 tag，且仍读不到 `wotb.parser.retry`/`wotb.parser.dlq`），`parser-worker` 的 read 保持 `^wotb\.parser$`（写仍由 `^wotb\.jobs$` 覆盖，ACL 不看 routing key）。`validate-plan.sh` allowlist 扩展至新地址并新增 fixture（新队列/binding create PASS、delete/update REJECT、ACL 放宽与越权读 REJECT、second plan no-op PASS），`deploy/test-rabbitmq-tofu.sh` 在真实 broker 上验证结果回流两个 routing key、nack 经 DLX 进 DLQ，以及双向越权读/声明拓扑被拒。**本 PR 不含任何生产者或消费者接线**，也不改 `wotb-web`。
- **MinIO Application Storage（双身份）**：新增 Maven 模块 `java/wotb-object-storage-minio`，实现**既有** `com.wotb.contracts.ObjectStorage` 端口（仅 `put`/`get`/`exists`，端口三方法不变、不新增 `delete`/`list`）——`MinioObjectStorage` 把全部 MinIO 异常包装成 `IOException`、不让 `io.minio` 类型出现在方法签名，`get` 的 `InputStream` 由调用方负责关闭；`MinioObjectStorageProperties` 是不可变 record，构造器校验非空白、`toString` 手写以**不打印 secretKey**（凭据只经进程环境注入，禁止落库/落日志）；`ObjectStorageKeys.tempJobObject(jobId, relativePath)` 是 `temp/jobs/<jobId>/<relativePath>` 布局的唯一事实源，遍历安全校验拒绝空白、`..`、绝对路径、反斜杠与控制字符。`io.minio:minio` 8.5.17 在 `java/pom.xml` 的 `dependencyManagement` 统一 pin（属性 `${minio.version}`），测试用 Testcontainers MinIO 模块（版本由 spring-boot-dependencies 的 testcontainers BOM 2.0.5 管理，不写 version；镜像显式 pin 到 Quay 上的 maintained tag，因为社区 `minio/minio` 已从 Docker Hub 下架）。OpenTofu `infra/tofu/minio` 新增第二个应用身份 `control_api`（TX 回放控制面）与既有 `worker`（Yecao parser-worker）保持**同前缀同权限**（`temp/jobs/*` 的 `s3:ListBucket` + `s3:GetObject`/`s3:PutObject`），`worker` 资源地址与策略文档逐字保留以避免 state 内资源替换；两个策略文档有意不抽公共 `locals`，因为改写已 apply 的 worker 策略会被 plan guard 判为 in-place update。`validate-plan.sh` allowlist 扩展至新 user/policy/attachment，拒绝规则不放宽（delete/replace/未知地址/任何 update 仍 REJECT），新增 `infra/tofu/minio/test-validate-plan.sh` fixture（新增身份 create PASS、second plan 必须全 no-op、delete/replace/未知地址/放宽策略 REJECT，并含 `minio_s3_bucket.hof_replays` 负例）。`deploy/minio-deploy.sh` 增加第二个身份的 required env 与 `TF_VAR_control_api_*` 导出，Deploy workflow 的 MinIO 目标与 `.env.example` 只增加变量名，CI MinIO smoke 在真实 apply 后按 state 断言两个身份存在、名字不同且都只持有 `temp/jobs/*` 策略。HoF 回放原件仍按 §14.5/§15.3 走 operator 卷迁移，本 PR **不引入任何 HoF replay 存储抽象**、不新增 `hof-replays/` 前缀。
...
- **TX RabbitMQ infrastructure ownership**：新增固定 `cyrilgdn/rabbitmq 1.10.1` 的 mirror-only OpenTofu root，管理 `/wotbtools` vhost、无 administrator tag 的 `control-api`/`parser-worker` identities 与其 vhost ACL。Compose 继续只拥有 broker runtime、持久卷、WireGuard-only AMQP 与 loopback Management API。TX 在本机 health 后执行 safety-guarded apply 和 clean second plan 才写 marker；CI 用 disposable RabbitMQ container 验证真实 provider 资源。PostgreSQL 仍是 job state 权威、RabbitMQ 只负责 delivery、MinIO 只作 temporary workspace。
- **TX RabbitMQ static topology ownership**：静态 RabbitMQ topology 改由 OpenTofu 单一拥有——`wotb.jobs`（topic、durable）exchange，`wotb.parser` / `wotb.parser.retry` / `wotb.parser.dlq` durable queues，以及 `parser.request` / `parser.retry` / `parser.dead` bindings；应用只 publish/consume 并保留 ack/nack、重试判定、幂等与 job state，启动时不得重复声明同一拓扑（否则 `PRECONDITION_FAILED`）。重试由 broker 队列 `x-message-ttl`（30000 ms）+ DLX `wotb.jobs` 回到 `parser.request` 实现，终态失败经 `parser.dead` 进入 DLQ；retry counter 与 job state 仍在 PostgreSQL。ACL 收紧为 `control-api` write `^wotb\.jobs$` / read `^$`（不可 configure、不可消费），`parser-worker` read `^wotb\.parser$` / write `^wotb\.jobs$`（仅为终态 DLQ publish 所需）；plan guard 按资源地址逐项比对 ACL（任何不同取值即 fail-closed，覆盖 `^wotb\..*$`、`^wotb.*$`、`^.+$`、窄但不正确的 regex、缺失 scope、control-api 越权读 `wotb.parser`）、拒绝 topology update/delete/replace、未知地址，以及应用身份获得任何 RabbitMQ tag（`management`/`administrator`/`monitoring` 等）、改名或未知 user attribute（tag 集合不可证明为空即 fail-closed，敏感 password 不作检查以便轮换）；disposable smoke 以真实 AMQP 验证 publish/retry/DLQ 与拒绝路径，并断言两个应用身份被 Management API 以 `Not management user` 拒绝。
- **TX Business PostgreSQL infrastructure**：新增与 `keycloak-postgres` 完全独立的 `business-postgres` 运行时（`postgres:18-alpine`、`business_postgres_data`、仅 `127.0.0.1:25432:5432`、`pg_isready` 健康检查、512m 上限），以及独立 OpenTofu root `infra/tofu/postgres-business`（`cyrilgdn/postgresql 1.27.0` 精确 pin + committed lock、mirror-only fail-closed、local root-only state）只管理 `wotb` 数据库、`control_api` 应用角色与 database-level grant（三者 `prevent_destroy`）。业务表仍由 Flyway 单一拥有；OpenTofu 不声明任何表/索引/序列，也不导入任何生产数据。新增 `validate-plan.sh` 计划安全策略（create PASS、in-place 应用密码轮换 PASS、delete/replacement/未知地址/非预期 update REJECT、second plan 必须完全 no-op）、`business-postgres`-only 选择性部署隔离（不需要 Keycloak/RabbitMQ/frontend/Caddy 输入或应用镜像元数据）、`infra/tofu/postgres-business/**` 只选择该部署路径不重建镜像，以及 `pg_dump` + SHA-256 + 恢复到一次性库的备份/恢复 runbook 与 disposable PostgreSQL 18 smoke。只读 `PRE_CUTOVER_READY` 门禁同样要求 Business PostgreSQL 就绪（容器 healthy、`pg_isready`、端口严格 `127.0.0.1:25432:5432`、marker 内容精确匹配），否则输出 `PRE_CUTOVER_NOT_READY`。
- **手工 Deploy 的 TX service 选择改为下拉菜单（`tx_services` = `choice`）**：`workflow_dispatch` 的 `tx_services` 由自由文本 `string` 改为 `choice`（`business-api` / `rabbitmq` / `business-postgres` / `keycloak` / `keycloak-postgres` / `wotb-frontend` / `caddy` / `all`，缺省 `business-api`），消除 production 手工发布时输错 service 名的空间；`target` 下拉（`tx` / `minio` / `parser-worker`）不变。新增 `all` 一键选择：在 manifest step 内展开为 `keycloak-postgres,business-postgres,rabbitmq,keycloak,wotb-frontend,business-api`（与 `deploy/tx/deploy.sh` 既有 `all` 选择集一致，字面值 `all` 绝不进入 `WOTB_DEPLOY_SERVICES`），`caddy` 保持「仅显式选择」——它由被选中的代理服务自动刷新，放进 `all` 会让整运行时发布额外依赖 Caddy 凭据。因为下拉本身不接受任意字符串，手工 TX 步骤不再解析逗号分隔列表；未知或空值仍 fail-closed 拒绝（`workflow_dispatch` API payload 可绕过 UI）。`workflow_run` 自动部署、immutable release/tag、MinIO / parser-worker 手工发布、OpenTofu 安全门禁、DNS/cutover 行为均不变。

### Architecture
- **TX-internal API routing**：公开 API 流量现在在 `wotb_tx_internal` 内终结（frontend nginx → `business-api:8087`），**不再有任何公开流量经过 Yecao `10.20.0.2:8087`**；WireGuard 只剩 TX→MinIO `9000`、TX↔RabbitMQ `5672` 与按需观测。Yecao 的 `wotb-backend` 容器及其 `10.20.0.2:8087:8087` bind 在退役窗口内仍保留（回滚与 §15.4 约束），但已无任何路由、探测或门禁依赖它；其彻底清除属 PR J 的归零检查与 operator 退役动作。
- **TX business runtime ownership**：backend 镜像的发布目标改为 TX `business-api`，Yecao `wotb-backend` 仅作为退役期显式选择保留。公开路由（nginx `/api/` 与 `location /api/replay/analyze`、Caddy）本 PR **不改**，仍指向 Yecao；一次性路由切换与 `TX_BACKEND_UPSTREAM` 硬校验替换由后续 PR H 完成。
...
- **TX pre-cutover runtime gate**：Yecao backend 业务端口仅绑定 `10.20.0.2:8087:8087`；TX deployment-owned health-probe 独立验证 WireGuard backend、frontend、Caddy 与 Keycloak。只读 `PRE_CUTOVER_READY` 门禁要求 official QQ `idp-qq=READY` 的实际 Admin API representation，并明确 DNS/Yecao retirement 仍需后续人工批准。
- **TX secret injection boundary**：TX Compose 与 TX-local OpenTofu 仅接收 GitHub Actions Secrets/Variables 通过 SSH 注入的进程环境；不再依赖服务器 `/etc/wotb/*.env` 文件。TX deploy 在修改 runtime 前 fail-closed 检查 Docker/Compose、`wg0` 地址与 WireGuard 路由；backend `8087` 动态可达性只在 application deploy/PRE_CUTOVER_READY runtime gate 验证。
- **Optional TX sponsor content**：赞助配置与二维码是可选静态内容；TX deploy 不再把 `sponsor-config.json`、赞助资源目录或 Android release 目录作为 staging hard requirement，缺失时沿用前端 graceful fallback。
...
### Fixed
- **手工 MinIO / Parser Worker 发布的 immutable release identity**：这两个手工入口此前在 Deploy workflow 里各自复制一份 release identity，且产出的 manifest 仍带 `imageTag: latest`；一旦该 manifest 走进 `release_plan.py validate` 的严格校验（只接受 `sha-<12>`），手工发布就会在部署前被判非法。现统一改为调用 `deploy/release_plan.py manual` 推导 identity，manifest 固定 `sha-<当前 main 的 12 位 SHA>`，并移除允许 manifest 使用 `latest` 的 `--allow-latest` 逃生口。同时补齐缺失镜像时的可执行指引：MinIO / Parser Worker 在进入 SSH/SCP 前就报出「先手工运行 Build workflow 的 `service=minio` / `service=parser-worker`」，三个实际部署 job 一律以镜像存在校验成功为前置条件。Build 仍照旧发布 `latest` convenience tag，任何 production Deploy manifest 都不再引用它，也不会回退到旧镜像。
- **MinIO endpoint 归一化**：`MinioObjectStorage` 现在把裸 `host:port` 归一为 `http://host:port` 后再交给 MinIO SDK。生产/Tofu 侧的地址形态就是 `10.20.0.2:9000`（`.env.example`、`infra/tofu/minio` 与 compose 一致），而 SDK 对无 scheme 的 endpoint 直接抛 `invalid hostname`——该缺陷在 PR B 之后第一个真实消费方（PR D 的 parser worker 管道测试）才暴露，因此修在共享适配器上而不是让每个调用方各自拼 scheme。
- **TX Keycloak web client production parity**：`infra/tofu/keycloak` 的 `wotbtools-web` 现按 pinned provider 5.9.0 schema 显式声明生产浏览器客户端行为——`login_theme = "wotbtools"`、`always_display_in_console = true`、`frontchannel_logout_enabled = true`、`consent_required = false`、`pkce_code_challenge_method = ""`，并通过 provider 官方 `extra_config` 声明 Keycloak 26.6.4 的 `frontchannel.logout.session.required = "true"`（provider 无 typed field，且 Keycloak 默认值同为 true），不使用 `local-exec`/provisioner；`wotbtools-admin-api` 与 Keycloak 内置 client 不受影响，`prevent_destroy` 保持不变。
...
- **GitHub Actions Maven repository isolation**：CI Maven 执行改用不配置 mirror 的 `java/settings-ci.xml`，避免 GitHub-hosted builds 被强制路由到 Aliyun；本地开发继续保留 `java/settings.xml` 的 Aliyun mirror。
- **CI dependency-resolution resilience**：Android CI 保留 `google()` / Maven Central 与受控的 Aliyun public fallback，并对 Gradle dependency-resolution 命令及 Observability 三个固定镜像的显式 `docker pull` 增加仅针对明确 transport failure 的有限重试；repository 顺序本身不承诺对 429/5xx 自动 failover，编译、测试与配置校验失败仍 fail-fast。Android debug assemble 与 JVM unit tests 在同一个 Gradle invocation 中执行，仍要求两个 task 都成功。未修改 dependency versions、业务代码或既有 PR314 OpenTofu 逻辑。
...
### Frontend
- **Showcase Footer 对比度修复**：沉浸界面的 Footer 使用基于 `--bg-card` 的半透明暗色底板与轻微模糊，普通文案、ICP 备案链接与顶部边线在复杂地图背景上保持可读；Classic 界面、Footer 结构与布局不变。
...
### CI/CD
- **Deploy application probe 与 schema-aware rollback 修复**：生产 application gate 改由同一 `wotb_internal` network 中 deployment-owned `health-probe` curl service 独立检查 backend、带 `Host: wotbtools.com` 的 frontend nginx 与 Keycloak OIDC，不再依赖 application image 的 `wget/curl`，失败日志保留 DNS/connection refused/timeout/non-2xx 原因。完整自动部署将 Flyway 最新成功 version 写入 `DB_SCHEMA_VERSION.lkg`；回滚前若数据库 schema 已超过 LKG 支持版本则输出 `ROLLBACK UNSAFE`、保留 candidate runtime 并拒绝旧 backend，未带 metadata 的 legacy LKG 仅允许从健康 live deployment 安全 bootstrap。已有 production state 时双库 pre-deploy backup 缺失或失败会在 compose/Flyway mutation 前 fail-closed，真实 first-install 保留明确 skip reason。验证：normalized `deploy/test-deploy-rollback.sh` 全部通过；未宣称真实生产 deploy/rollback 成功。
- **Legacy LKG production recovery**：补充受控的一次性 operator-confirmed schema bootstrap，允许当前旧 V21 live image 不健康且数据库已是 V22 时显式记录旧 LKG 的 V21 compatibility；不从当前 DB 推断、不执行 Flyway downgrade。candidate 失败时 schema-aware rollback 继续拒绝 V21 backend 并保留 candidate runtime；Deploy smoke fixture 同步模拟生产安装后的可执行 backup helper。
...
- **Keycloak production runtime hardening**：Keycloak 构建阶段固定 PostgreSQL，生产编排统一使用 `start --optimized`；新增真实 Docker runtime smoke，验证应用 OIDC discovery、无 management health/metrics 配置、无宿主机管理端口暴露及无启动时 augmentation。
...
- **Production deploy capability-aware rollback**：rollback verifier 固定由当前 deployment runner 持有，历史 LKG 通过 capability metadata 执行 core rollback gate；application gate 只依赖 backend、frontend/nginx 与 Keycloak OIDC discovery，不再因已移除的 management capability 或 verifier 文件而二次失败。
