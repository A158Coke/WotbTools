...
### CI/CD
- **Backend Docker build cache correctness**：生产 Backend/Frontend/Keycloak/MinIO BuildKit GHA cache 改用稳定的 image-specific scope；PR Backend build 复用 Buildx GHA cache，Dockerfile 的 Maven dependency preparation/package 将依赖仓库写入可缓存的 BuildKit layer 并对依赖解析失败 fail-fast。Host Maven cache 仍保留给 CI 的 Maven test，不把 `.m2repo` 注入镜像。
