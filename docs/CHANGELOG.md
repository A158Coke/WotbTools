...
### Added
- **Team Autopsy 限用于团队复盘（team perspective）**：战犯/MVP 判断只应用于训练房/联赛团队复盘（`TeamReplayAnalysisService` 单团队单元），随机战斗个人复盘（`TacticalReviewHarness`）不再输出团队剖析段。玩家身份改用无业务推断的 `playerKey`（P1..P7，按本方 roster 稳定编号，同队同名坦克可区分）：Prompt 用 playerKey 引用、Parser 验证 playerKey 全部属于本方 roster 且不重复、contribution 仅 HIGH/MEDIUM/LOW/UNKNOWN、confidence 仅 EXACT/INFERRED/PARTIAL/UNKNOWN、每条 verdict 引用有效 playerKey 且至少一条 evidence、判胜至少一个 MVP / 判负至少一个战犯、空结果拒绝，任何契约不成立整段返回 null；最终渲染按 playerKey 回查后端权威昵称/坦克名。`TeamAutopsyStatsBuilder` 只构建 recorderTeam 本方玩家，weakOutput 均值只使用本方队伍（敌方伤害不影响）；权威结算与派生 flag 置信度分离（earlyDeath/weakOutput 为规则候选、deathInCriticalWindow 继承窗口 confidence 且结算级代理不得 EXACT）；死亡时间线仅含本方。Call #3 预算按整体剩余时间裁剪（min(30s, 剩余 - margin)，不足安全余量不启动并记录 budget_exhausted），`AI_CANCELLED` 重新抛出不被吞掉。
- **AI Review 团队剖析（Team Autopsy，第 3 次调用）**：判负 → 主要战犯（≥1，可多人，按证据类别：短窗口掉血过多 / 过早阵亡 / 缺乏输出 / 脱节）；判胜 → MVP（≥1）。输入 = 权威逐人结算 + Call #1 赛前职责基线 + 关键窗口；30s 独立 stage budget，失败/解析失败不影响主复盘；展示追加到 `analysis` 文本尾部「团队剖析」段，`AnalyzeResponse` 结构与前端零改动。
