...
### Replay / Auth
- **Android pending replay 字节 transport 改为同源 synthetic HTTPS 资源（生产 hotfix）**：PR286 删除 `onShowFileChooser` 注入后，external replay 只剩 Native Bridge 单一 ingress，但 Web 侧仍 `fetch(pending.uri)`，而 `pending.uri` 是 FileProvider `content://`。WebView 保持 fail-closed（`allowFileAccess=false` **且** `allowContentAccess=false`），`allowContentAccess=false` 会挡掉 content scheme —— 真机表现为「登录成功 → authenticated=true →  `getPendingReplay()` 成功 → `fetch(content://…)` 失败 → catch → 不建 Processing Job、不 ACK」，用户看到「登录成功但没有自动上传」。修复**不放宽** WebView：wire 上的 `uri` 改为固定同源常量 `https://wotbtools.com/__native/replay-pending`（新增纯策略 `PendingReplayResourcePolicy`，JVM 单测覆盖 exact match 与拒绝变体），由 `shouldInterceptRequest` 流式返回 app private cache 的字节（不 `readBytes()`、不 Base64、不整文件 ByteArray）。该 URL 不含 pendingId / 文件名 / 本地路径 / token / state / code（pendingId 继续只经 Native Bridge 传输，仍作 ACK identity 与 Processing `operationId`）。**fail-closed**：命中该 URL 的请求永远 Native-owned——无 pending → 404、backing file 缺失 → 404、读取异常 → 500，**绝不 `return null`** 放行到真实 nginx/backend。`PendingReplay` 随之不再持有任何 URI（连同 `fileProviderAuthority` 一并移除，FileProvider 仅保留给 APK 更新），也就没有本地路径泄漏面。读取失败现在**不静默**：`useNativeReplayImport` 新增 `onPendingFileError`，`ReplayWorkspace` 复用现有 replay processing error surface 显示三语文案，且不 ACK、不启动 Processing、不清 metadata，pending 保留可重试；日志补齐阶段化低敏记录（`pending discovered` / `stream fetch start|failed status=…` / `file constructed` / `processing accepted` / `ack success|stale`，Android 侧 `replay-pending stream requested|served|missing|failed`），只写前 8 位短引用。
