...
- **Multi-team 全局三层 Prompt Budget**：`TeamAiPromptBuilder.multi()` 改为全局三阶段输出——Phase 1 所有 included perspective mandatory header → Phase 2 所有高优先级 facts → Phase 3 所有 optional details；预算规划将每个 perspective 的 minimum high-priority facts 计入 included 判断；included 定义修正为 mandatory + minimum facts 都满足；新增 `AiPromptBudgetExceededException` 专用 domain exception（HTTP 400 映射）。
- **Authorization scheme 通用脱敏**：`redactNonJson()` 全面覆盖 RFC-style auth scheme（大小写不敏感 letter + alnum，任意长度 credential）；新增 6 个测试覆盖 mixed-case/lowercase/uppercase/short credential/反误报。
- **Omitted perspective 全链路契约**：`analyzeTeamGroups()` multi partition 的 top-level key events 只从 `includedUnitIds` 收集；omitted unit 的 key events 不进入 AI analysis；omitted unit 的 fact report 保留原始 key events 但带 `AI_PERSPECTIVE_OMITTED_FROM_PROMPT` limitation。
...
- **PR #39 最终契约收尾**：
  - **unavailableAnalysisUnitCount**：API 新增字段区分不可分析与 prompt omitted；数学关系 `total = analyzed + omitted + unavailable`。
  - **Multi-partition omission 聚合**：汇总各 partition omitted IDs，全局只生成一个 `PERSPECTIVES_OMITTED_COUNT_<TOTAL>`。
  - **Provider body 安全策略**：停止 broad 两词 regex 猜测；generic message 不再尝试自定义 auth scheme redaction。
  - **前端 limitation 本地化**：`AnalysisLimitations.vue` 和 `AnalysisUnitItem.vue` 统一调用 `localizeLimitation()`；`PERSPECTIVES_OMITTED_COUNT_N` 动态三语模板。
  - **Count UI**：`AnalysisSummary` 显示 identified/analyzed/omitted/unavailable 四个独立计数。
