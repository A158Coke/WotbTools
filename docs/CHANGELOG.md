...
### Changed
- **PR2 Replay Session state owner**：新增 `useReplaySession` 统一持有 selection、Processing/Result identity、Export state 与 Workspace view；`useReplayWorkspace` 改为 session facade，Export Job lifecycle 下沉至 `useExportJob`。保持现有 API、sourceId、single-flight、stale-response 与 Android replay 行为不变。
