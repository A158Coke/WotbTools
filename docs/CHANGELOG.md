...
### Added
- **车辆库全面切换 blitzkit + 10 级多炮拆分**：`update_tankopedia.py` 数据源由 WG 百科切换为 blitzkit（`assets.blitzkit.app/definitions/tanks.pb`，游戏客户端直出，含 WG 未收录的新车如 SPHT / AC Atlas），车辆名统一英文；每发弹输出 `shells`（type/damage/penetration，type 归一化 ap/apcr/heat/he）；10 级顶配炮塔多炮车按炮拆分为多条记录——主记录 key = tank_id（第一把炮，与 WG 默认配置逐辆核对一致），变体 key = `<tank_id>_<gun_id>`，每条带 `gunId`/`isDefault`；`extraKnowledge` 保留合并覆盖该车全部记录，任一记录丢失即失败；meta 增加 `tank_count`。`Update Tankopedia` workflow 同步简化：blitzkit 为公开 CDN，runner 直接同步提交，不再需要 `WG_APPLICATION_ID` / `VPS_*` secrets 与 IP 白名单。
