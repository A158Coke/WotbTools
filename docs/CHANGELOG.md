...
### Added
- **Wargaming.net 亚服登录（ASIA）**：新增 Keycloak 自定义 Identity Provider `keycloak-wargaming-provider`（Provider ID `wargaming`，一个类型可建多区服实例），走 WoT Blitz 官方认证/账号接口（ASIA→`api.wotblitz.asia`；EU/NA host 白名单与 region 配置项已就绪，实测 `application_id` 按 Blitz 游戏注册、跨区通用）；WG 登录创建独立账号（username = account_id、broker = `wg:{region}:{account_id}`，ASIA 实例即 `wg:asia:{account_id}`），重复登录自动刷新昵称与展示名；**登录身份安全绑定**：`POST /wot/auth/prolongate/` 服务端响应返回 token 所属 `account_id`（官方契约字段），broker/username/wotb.account_id 全部取自该可信值，浏览器回调的 account_id/nickname/expires_at 仅作一致性检查，杜绝「有效 token + 篡改回调 account_id 登录成他人账号」；JWT 新增 `wotb_region` / `wotb_account_id` / `wotb_nickname` / `wotb_verified(boolean)` claims；后端 `user_profile` 新增 `wotb_account_source` / `wotb_account_verified_at`（V12），支持 WG 区服自动写入与 `PUT /api/users/wotb-account/from-login` 幂等同步，**后端已兼容欧服/美服**（V13 `CHECK (wotb_server IN ('CN','ASIA','EU','NA'))`，创建/同步/只读逻辑按 JWT `wotb_region` 参数化）；前端新增登录选择页（QQ / Wargaming 三语），个人中心展示 Asia 服务器、Wargaming.net 来源与已验证状态，WARGAMING 来源资料不可编辑/解绑。存量用户已补 `region=CN`（138/138）。
