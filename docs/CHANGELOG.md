...
- **HoF ownership 解耦为 WotB 游戏账号**：新增 Flyway `V22__hof_ownership_by_wotb_account.sql`，把 `hundred_battle_submission` / `mark3_submission` 的 `game_account_id_snapshot` 重命名为 `wotb_account_id` 并删除 `user_keycloak_id`；partial unique index 由 `(user_keycloak_id, vehicle_id)` 改为 `(wotb_account_id, vehicle_id)`（百场 `uk_hundred_battle_pending_account_vehicle` / `uk_hundred_battle_current_account_vehicle`，三环 `uk_mark3_submission_active_account_vehicle`），查询索引改为 `idx_hundred_battle_submission_account` / `idx_mark3_submission_account`。账号列本就冻结自 `profile.getWotbAccountId()`，无需回填；单场 `hall_of_fame_record` 不变（一直按 `account_id` 归属）。
- **V22 冲突自愈（幂等）**：同 `(账号, vehicle, status)` 存在多条 active 记录时，保留 `submitted_at` 最新（并列取 id 最大）的一条，其余转终态——百场 `CURRENT` → `SUPERSEDED`、百场 `PENDING` → `DELETED`（`deleted_by='V22_OWNERSHIP_MIGRATION'`、`delete_reason='ADMIN_CORRECTION'`）、三环 active → `DELETED`；转终态的行在同一事务内删除其 replay evidence 行并清空 proof 截图。无冲突时影响 0 行，与干净迁移完全等价。
- **服务层 ownership 切换**：百场/三环 `cancelSubmission` / `userStatus` 先由 `user_profile` 解析当前登录用户绑定的 WotB 账号，再按账号判定归属（账号不匹配或未绑定 → 403 `HUNDRED_FORBIDDEN` / `MARK3_FORBIDDEN`；未绑定账号时 `userStatus` 返回三个空列表）。记录创建后用户改绑到别的 WotB 账号时，记录仍属于原账号，因此不可再取消。
- **Admin Users 服务端分页 + 合并数据源 + 批量删除**：`GET /api/admin/users` 响应由裸数组改为 `{items,page,size,totalItems,totalPages}`，旧的 `?limit=` 参数移除（取消 `limit=200` 假分页）；新增 `segment=keycloak|local` 与 `idpAlias`——`segment=keycloak`（默认）以 Keycloak realm users 为权威源，没有任何本地 profile 的 Keycloak-only 用户也能被找到并删除（旧 Juhe QQ cleanup 前提）；`segment=local` 以本地 `user_profile` 为权威源，暴露 Keycloak 侧已不存在的孤儿绑定（`keycloakUserMissing=true`），删除它可释放 `(wotb_server, wotb_account_id)` 唯一槽位，传 `idpAlias` → 400 `IDP_FILTER_REQUIRES_KEYCLOAK_SEGMENT`。列表行 DTO 改为 `AdminUserListItemDto`（旧 `AdminUserDto` 删除），新增 `POST /api/admin/users/bulk-delete`（`{userIds, confirm}`；`confirm` 缺失/false → 整批 400 `CONFIRMATION_REQUIRED`；每用户独立事务，允许 partial success）。
...
- **新增错误码与契约同步**：`util/ErrorCode.java` 新增 `BULK_LIMIT_EXCEEDED`、`INVALID_USER_SEGMENT`、`IDP_FILTER_REQUIRES_KEYCLOAK_SEGMENT`。`contracts/http/openapi.yaml` 中百场 admin 的 `gameAccountIdSnapshot` 改名为 `wotbAccountId`，并新增 `POST /api/admin/hof/hundred/submissions/bulk-delete` 与 `BulkDeleteModerationRequest` / `BulkDeleteItemResult` / `BulkDeleteResult` schema；mark3 / 单场 / admin-users 三个家族本就不在 `openapi.yaml` 覆盖范围内（既有状况，本次未补全）。
- **IAM ≠ HoF 不变量**：HoF 数据属于 WotB 游戏账号而非 Keycloak 用户；仓库中没有任何 FK 指向 `user_profile`（全仓唯一的 `on delete cascade` 在 `V3__create_boosting_tables.sql`，boost 域内部），删除 Keycloak 用户不连带删除 HoF 行。因此删除用户必须走 WotBTools admin API（先删本地 profile 再删 Keycloak 用户），绕过它直连 Keycloak 会留下孤儿 profile 并阻塞重绑，需用 Admin Users 的 `segment=local` 清理。
