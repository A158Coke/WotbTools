...
### Changed
- **FE ↔ BE HTTP contract infrastructure**：新增 OpenAPI 3.1 wire contract 作为 HTTP 唯一事实源，生成前端 transport/schema/error-code registry，加入 OpenAPI/ref、generated drift、生产形状 fixture、Ajv runtime 与 Playback serialization 的独立 CI gate；补齐 domain/transport/artifact compatibility 与 ApiError 维护规则。Playback 旧 artifact 仅在读取边界兼容，live response 不放宽为 legacy enum。
