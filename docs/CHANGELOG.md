...
- **CI dependency-resolution resilience**：Android dependency resolution 保留 `google()` / Maven Central 为主要仓库，并增加受控的 Aliyun public fallback；Android debug assemble 与 JVM unit tests 在同一个 Gradle invocation 中执行，仍要求两个 task 都成功。未修改 dependency versions、业务代码或既有 PR314 OpenTofu 逻辑。
