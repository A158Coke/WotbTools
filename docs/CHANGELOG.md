...
### Replay processing
- 新增默认跳过、显式 `-Dperformance=true` 才运行的 replay core 性能基准：递归发现 `common/data`，无数据时回退到三份 committed fixtures；按 archive/parser/reconstruction/full 分阶段测量，支持并发矩阵、确定性 SHA-256 facts fingerprint、JSON/CSV/Markdown 报告与 JFR。
