...
### Changed
- **Pre-Dual-Cloud code foundation**：新增无 Spring/provider SDK 依赖的 `wotb-contracts` 与独立 `wotb-control` artifact，建立 metadata-only async ports、显式 current `READY` ↔ future `SUCCEEDED` adapter、受保护的 JdbcClient DB probe，以及 JVM/Java 25 Native feasibility 记录；RabbitMQ/COS/AI/Replay extension 保持延期，当前 Web/Android contract 不变。
