...
- **直接 GitHub Runner → TCR 发布 PoC benchmark（仅手工触发，不改生产发布链）**：TX workload 的现有发布链是 Build → GHCR → OCI archive → rsync 到 TX → `docker load` → TX 侧 `docker push` 到 TCR；run `35670563899` 三次 900s rsync 全部超时（`ERROR: rsync transport failure persisted after 3 attempts`），而 TX → GHCR 反向拉取在 171.1 MB 层上只取到 44.97 MB、56m16.97s 后 `connection reset by peer`，因此 OCI 传输与 GHCR 都不是 TX workload 的可靠镜像通道。新增 `.github/workflows/benchmark-tcr.yml`（**仅 `workflow_dispatch`**：无 push/pull_request/workflow_run，因此永不接力 Deploy、不产出 `deployment-manifest`、不动生产 Compose），用 `docker/setup-buildx-action` + `docker/build-push-action` 把**真实** `docker/Dockerfile.backend` 以 BuildKit registry exporter 一次构建直接推送到 `<vars.TCR_REGISTRY>/<vars.TCR_NAMESPACE>/wotbtools-backend:benchmark-<run id>`：不含 GHCR 发布、不含 OCI tar、不含 rsync/scp/SSH/`docker load`/`docker pull`，不调用 `transfer-oci-to-tx.sh` 或 `publish-loaded-image-to-tcr.sh`；标签既不是 `latest` 也不是 `sha-<12>`（生产 Deploy 只解析这两个），因此不构成任何 release identity，也不削弱 immutable image 契约。凭据沿用 Deploy 的仓库级 `TCR_USERNAME`/`TCR_PASSWORD` 与 `vars.TCR_REGISTRY`（`deploy.yml` 的 `image_existence` job 在无 `environment` 的 job 内已证明其仓库作用域），**不挂 `environment: tx-production`**——该 environment 无 protection rule，挂上只会给一次 benchmark 留下生产 deployment 记录。可观测性：`stage=benchmark-identity/benchmark-start/build-publish-start/build-publish/verify-publication/benchmark-end/benchmark-total` 输出 `duration_seconds` 与 `readback_seconds`，BuildKit 的 plain 日志在同一 step 内区分编译 step 与 `exporting`/`pushing` step，从而把 BUILD 延迟与 REGISTRY EXPORT/PUSH 延迟分开；首跑**刻意无 cache、无任何重试包装**（不引入 `is_transient_*` 式重试），原始失败文本原样保留以便归类 DNS/TCP/TLS/auth/registry API/blob upload/manifest/timeout/connection reset/rate limit。发布验证只用 `docker buildx imagetools inspect --format '{{.Manifest.Digest}}'` 做 manifest 读回（绝不把镜像 pull 回来），要求 build 输出 digest == TCR 读回 digest 且形状为 `sha256:<64 hex>`，成功时输出 `TCR_TARGET=`/`TCR_TAG=`/`TCR_DIGEST=`/`TCR_PUBLICATION=PASS`（不含任何凭据）；digest 不一致被显式标注为 identity 结果而非 TCR 网络结果。`scripts/ci/test-workflow-contract.sh` 新增该 benchmark 的安全契约：trigger 恰好是 `workflow_dispatch`、`permissions` 恰好 `contents: read`、不得挂 `environment`、`uses` 恰好是 checkout/TCR login/setup-buildx/build-push、step 文本不得出现 `ghcr`/`transfer-oci-to-tx.sh`/`publish-loaded-image-to-tcr.sh`/`rsync`/`scp`/`ssh`/`docker load`/`type=oci`/`upload-artifact`/`deployment-manifest`/`crane`/`latest`/`retry`/`continue-on-error`、tag 必须来自 `benchmark-$GITHUB_RUN_ID` 且以当前 main HEAD 为前提，并继续断言 GHCR+OCI+TX 发布链与两个 TX helper 仍然存在。验证：`bash scripts/ci/test-workflow-contract.sh`（含 5 个 mutation 负例：push trigger、GHCR login、`latest` tag、`environment: tx-production`、retry action 全部被拒绝）、`deploy/test-build-workflow.sh`、`deploy/test-tx-publication-helper.sh`、`scripts/ci/test-transfer-oci-to-tx.sh`、`deploy/test-release-plan.sh`、`scripts/ci/test-path-filters.py` 全绿；本次**不执行 benchmark run**（首跑由 operator 手工触发），生产发布链、`transfer-oci-to-tx.sh`、`publish-loaded-image-to-tcr.sh` 与 GHCR 发布路径全部未改动。
