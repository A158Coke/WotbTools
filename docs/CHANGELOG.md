...
- **Replay Processing Job PostgreSQL 权威状态**：新增 Flyway `V23` 三张状态投影表（`replay_processing_job` / `replay_processing_source` / `replay_processing_operation`）与 `ReplayJobAuthority`（`JdbcClient`，不引入 JPA 实体图），job/source 生命周期与 operationId 幂等不再只存在于单实例内存。注册表新增 `wotb.replay.processing-job.repository=memory|jdbc` 开关（缺省 `memory`，既有部署运行时行为逐字不变）：`jdbc` 模式下每次状态迁移 write-through 覆盖整行投影，内存未命中时从权威状态恢复只读投影，因此进程重启后 job/source 状态仍可读，TTL sweeper 同时回收过期终态投影，启动孤儿清理改用权威 job 集合以免误删可恢复 job 的本地产物。权威语义上有四条硬约束：① 一次 `save` 在单事务内完成 job 行 + source 投影，任一步失败整次回滚（不会留下新 job 状态 + 残缺 source 行）；② 每次迁移在 job 的**同一监视器边界内**把 `revision` 与它描述的状态/来源/计数器捕获成一个不可变快照（`ReplayJobPersistenceSnapshot`），落库只以该快照为输入、绝不回读可变状态——因此不存在「旧状态配新版本号」的错配；UPSERT 仅在 revision 更大时生效，陈旧/乱序写入被拒绝且不改写 source 行；③ 持久化失败 **fail closed**——创建失败即创建失败（不登记 job，并清理已落盘输入），迁移失败则把 job 从内存视图驱逐并抛出，绝不对外声称数据库未提交的状态，终态同样必须落地；④ operationId 的 COMMITTED 由 PostgreSQL 裁决并参与 claim 线性化：重启后先识别已提交 identity 再决定是否成为 creator，跨实例并发 creator 只产生一个权威 `jobId`，loser 收敛到胜者 `jobId` 并协作取消自己的重复 job；冲突后有界重读仍解析不到权威 winner 时，取消 loser 并以稳定错误码 fail closed（`PROCESSING_OPERATION_IDENTITY_UNRESOLVED`），**绝不返回没有权威 operation 映射的 loser jobId**（否则后续重试会因查不到 committed identity 而重复创建）。`IN_FLIGHT` reservation、`entries`、`ProcessedDataset` 与 artifact 内容刻意不落库：执行上下文不可持久化，Dataset 读取归属对象存储。
