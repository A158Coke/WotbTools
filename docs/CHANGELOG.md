...
### Fixed
- **死亡时刻 provenance 严格化 + EntityMethod 版本门禁（P0-2/P0-3）**：
  ① `PlayerResultFormat.deathSec()` 改为严格 `LIVE_EXACT → survivalTimeSec / SETTLEMENT_SECOND → deathTimeMillis / UNKNOWN → 0`，**删除**裸 `survivalTimeSec`/`deathTimeMillis` fallback——`deathTimeSource=UNKNOWN/null` 时即使残留旧字段也不得偷渡成 KNOWN（所有 Trade/KAST/League/AI 死亡时刻消费方的唯一入口）。已知死亡由 source 显式声明：`LeagueRatingConflictDetector.reconcileDeathTimes` 收口到 KNOWN 时同步写 `SETTLEMENT_SECOND`+`deathTimeMillis`；core/web 全部 fixture builder 补 canonical `deathTimeSource`。
  ② `ReplayVersionGate` 新增 `methodLayoutAllowed`（11.19 current + 11.18 legacy only）；`EntityMethodDecoder` 对 method0/1/5/17/20/27/29（legacy-compatible 观测布局）加版本门禁——unknown/future 版本 raw-preserve + VERSION_UNSUPPORTED，不再无条件产出 EXACT。补 `PlayerResultFormatTest`/`ReplayVersionGateTest`/`EntityMethodDecoderTest` 回归。
- **回放死亡权威链 / 版本门禁 / method38 配对修复（PR162 review blockers）**：① 死亡权威链改为 `LIVE_EXACT > SETTLEMENT_SECOND > UNKNOWN`——`DeathTimeReconciler` 对**所有**阵亡玩家先查最终 live EXACT terminal（`PlayerResult.deathTimeSource` + `PlayerResultFormat.deathSec` 按来源消费），sub-second 精确时刻覆盖结算秒级；早期死亡被复生否决时回退结算，绝不再让结算压过 live 证明。② 版本门禁收紧：PR147 closed semantics（method38 low16 / modifier / component namespace / method36 field semantics）**仅**对 `11.19.0_china*` 启用，11.18 保留 container/settlement/基础事实（Type10/HP/弹药/弹丸）legacy-compatible 解码，unknown/future raw-preserve 不 crash；删除 `isCurrentFamily` 死代码。③ method38↔recorder shot 配对改 deterministic exact rawClock + sequence 顺序（同刻多发射也能逐发关联），±1s 仅作严格唯一 fallback，ambiguity=UNKNOWN，一个 result 不被多个 shot 复用。
...
### Added
- **回放业务 production convergence（PR147 协议事实生产化）**：把 PR147 已证明的 11.19 回放协议事实沉淀为生产层语义——协议只解析一次、事实只定义一次，赛果解析 / 战局回放 / AI 复盘三个板块消费同一份 canonical facts。
  - **Protocol Core**：`ReplayPacketParser` 与 `ReplayPacketStreamReader` 改为 strict contiguous framing（zero-length payload 合法、terminator=type 0xFFFFFFFF、拒绝越界/尾部垃圾、禁止 byte-by-byte resync，恢复扫描仅保留为 `LegacyDeathHeuristicDiagnostics`）；新增 `ReplayVersionGate`（`11.19.0_china*` + 11.18 legacy 兼容，未知版本 raw-preserve + diagnostics）；Type10 完整 49 字节 decoder（`attachmentParentEntityId` / `positionErrorX/Y/Z` / `trailingStateRaw`，尾部语义=UNKNOWN，绝不命名为 onGround/isError）；物理坐标契约 1 unit≈1m（speed m/s·km/h / forward-reverse / yaw rate derived facts）。
  - **HP / Death / AoI / Movement**：死亡权威链 `LIVE EXACT → SETTLEMENT_SECOND → UNKNOWN`（`PlayerResult.deathTimeSource` + `DeathTimeReconciler`；删除 EntityLeave / 最后位置 / 伤害阈值等 legacy 死亡启发式权威）；`HpObservation`/`ReplayHpTimeline` 统一 HP 来源（recorder AFFIRMED / ally OBSERVED_EXACT / enemy fail-closed UNKNOWN；Tankopedia 仅作 `tankBaseHp` base reference）；`ReplayAoiLifecycle`（Type4 收段 / Type33+5 重入 / UNKNOWN_AOI，禁跨 AoI gap 插值）；`MovementSegment` 升级（distance/duration/speed/state/vertical/yawRate）。
  - **Combat Facts**：`ShotFact`/`ShotLifecycle`（method0→29→20→27→38，recorder shooter 过滤）、`ShotResolution` 完整 low16 位映射（含 0x0200 组件未击穿）；method38 component/crew 状态（31-43）；modifier [1,2] 组合；弹药 Type28/method17/descriptor version-aware；`TargetingInfoSnapshot`（field1-5/field6.1 物理角色 PROVEN）+ `TargetingDerivedFacts`。
  - **Consumer 迁移**：`PlayerEvidenceFormatter` 新增 `RECORDER_COMBAT_FACTS_BACKEND_COMPUTED` 渲染 canonical 射击/命中事实；`PlayerResultFormat.deathSec` 统一消费方；Rating/导出只消费 canonical DTO。
  - 文档：`docs/reference/replay-parsed-fields.md` 修正 `errorFlag→trailingStateRaw`、死亡权威链与 `deathTimeSource`；各 production docs 加 AFFIRMED/UNKNOWN 状态标记。
  - 测试：新增 Protocol framing / Type10 / AoI / Death / HP / method38 / method36 / ShotLifecycle / TargetingDerivedFacts / RecorderShotEvidence 等回归；真实 fixture 回归通过。
