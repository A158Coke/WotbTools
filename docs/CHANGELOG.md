...
- **PR188 Known-Bugs 收尾：Replay Workspace 共享 selection + Battle Playback 反未来泄漏 + 战斗装载本地化**：
  ...
  - **战斗装载本地化（Plan §21–§23）**：后端只返回稳定 `logicalItemId` / `wireCode` / numeric `equipmentId`，用户可见文案统一由前端 `src/data/loadoutItems.js`（zh/en 取自 `common/wotb-item-catalog-json/` authoritative 名称，ru 采用官方游戏术语）＋ i18n 提供。`V2VehicleInspector` 不再裸显 `MULTI_PURPOSE_RESTORATION_PACK` / `REPAIR_KIT` / `103` 等 raw 内部 id；未知 consumable/provision/equipment 走三语「未知消耗品/补给/装备（id）」fallback 并保留 raw id 仅诊断。
  - 验证：frontend targeted（`battlePlaybackV2` / `ReplayWorkspace` / `useReplayWorkspace` / `ReplayPage` / `BattlePlayback` / `V2VehicleInspector` / `loadoutItems`）全绿；`npm run build` 通过。
