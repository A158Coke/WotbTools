...
### OpenTofu
- 新增独立 Grafana OpenTofu root，纳管现有 9 个 dashboard 并保留 dashboard JSON 为 canonical source；Prometheus/Loki datasource 因 Grafana read-only 限制继续由 file provisioning 管理，trusted workflow 使用 `GRAFANA_PAT` 只读 plan，禁止自动 import/apply。
