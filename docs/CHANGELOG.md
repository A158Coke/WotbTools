...
### Added
- **随机战斗 AI 复盘友方/敌方语义**：新增 `PlayerSideResolver`（FRIENDLY/ENEMY/UNKNOWN）、`FriendlyEnemyResult`（FRIENDLY_WIN/ENEMY_WIN/DRAW_OR_UNKNOWN）与 `PlayerAnalysisPromptFormatter`；随机战斗 AI Prompt 不再使用"队伍1/队伍2"，改为"友方/敌方"；胜负使用完整三态，平局/未知不作为失败记录；胜率只统计已知胜负场数；录像者在 raw team 2 时仍正确识别为友方；多人分析每场独立解析 recorder；`PlayerResult.team` 原始编号不受影响。
