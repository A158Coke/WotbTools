...
- **Caddy-only TX 部署不再要求无关应用镜像的 immutable identity**：`deploy/tx/deploy.sh` 原先在 `stage_and_validate` 中无条件为 wotb-frontend/keycloak/business-api 解析 immutable identity（`current_or_target_tag`），因此 `WOTB_DEPLOY_SERVICES=caddy` + `WOTB_DEPLOY_IMAGE_SERVICES=` 会在触碰 Caddy 之前以 `current immutable image identity is unavailable for wotb-frontend` 中止（run `35648585435`：生产 `tx-production-release.json` 只有 business-api/keycloak）。现在 identity 要求由「本次部署是否真的启动应用镜像服务」决定（`starts_application_image_services`：`all` / `keycloak` / `wotb-frontend` / `business-api`）：Caddy-only 与既有数据库/RabbitMQ 类选择器一样，只为本次不启动的服务渲染 incoming immutable release tag（不写 release metadata、不伪造 identity、promoted Compose 不留 `${TAG}` placeholder），并且 Caddy-only 选择器不再重建 frontend 容器——`up -d --no-deps --force-recreate caddy` 之后改用 `docker compose exec -T wotb-frontend nginx -t` + `nginx -s reload` 在**运行中的**容器内就地重新解析 `set_real_ip_from caddy`（保留 nginx 按真实客户端 IP 限流的边界，同时不拉取、不重建任何应用镜像，也就不需要它们的 identity）。真正部署应用镜像服务的选择器保持 fail-closed：metadata 与 live Compose 都无记录时必须在启动任何容器之前报错。`deploy/test-tx-runtime-config.sh` 新增回归：不完整 metadata + Caddy-only 必须完成、只重建 Caddy、frontend 只做 in-place reload、promoted Compose 仍 pin immutable tag、不得写入 frontend metadata；同一状态下 `wotb-frontend` 部署仍必须失败且不启动任何服务。
