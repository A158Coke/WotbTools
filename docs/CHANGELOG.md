...
- **遗留生产资产清理清单（PR J / J1，inventory only）**：新增 `docs/operations/legacy-retirement-inventory.md`，把 Yecao 遗留应用栈退役的全部资产逐项盘点并分类为 KEEP / DELETE / ARCHIVE / HUMAN，每条给出真实路径、当前引用面（runtime / CI / 回滚 / operator）、删除风险与建议阶段。盘点同时给出三条**决定阶段顺序的硬约束**：① TX 只读门禁 `yecao-backend-wireguard-bind`（`deploy/tx/deploy.sh:1616-1651`）**主动要求** Yecao `wotb-backend` 的 `10.20.0.2:8087:8087` bind 存在，因此在解耦之前任何退役动作都会让门禁 FAIL；② 观测验证链路运行在 `wotb-backend` **容器内部**（`deploy/verify-observability.sh` 的 Grafana API helper 与 Loki canary），因此观测解耦必须先于容器停止；③ Prometheus job / Alloy stream / 6 个 Grafana dashboard / `ci.yml` 断言 / `ObservabilityDashboardContractTest` 把「Yecao backend 一直被采集」写成不变量，翻转它需要一次明确的可观测能力决策（TX 侧没有 Prometheus/Loki/Alloy）。清单同时确认计划的 8 条归零检查中已有 6 条在 `main` 上真实为零（`frontend/src/**`、`android/**`、`contracts/**`、`infra/tofu/**` webhook、workflow 中的 Yecao 地址、Keycloak callback），并把 J2–J7 的拆分与顺序约束（J2/J3 必须早于 J4；J5 为 operator 容器与卷动作，卷不删）写成提案。**本 PR 不改任何 runtime 行为、不删除任何资产、不触碰服务器与卷。**
