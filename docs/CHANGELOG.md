...
### Architecture
- **Frontend + Keycloak immutable release identity bootstrap**：Frontend 现有 build metadata 的 canonical 字段明确为 `buildCommit`；Keycloak 镜像通过 frozen release SHA 构建参数注入 `WOTBTOOLS_BUILD_COMMIT`，启动时输出该 identity。Build workflow 继续只为 affected services 发布 immutable `sha-<12>` 与 `latest`，本次路径选择预期为 Frontend + Keycloak，backend skipped；未修改认证行为、Deploy fail-closed、数据库或备份链路。
