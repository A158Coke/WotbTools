...
### Changed
- **Battle Playback temporal/loadout closure**：active-battle playback now excludes negative-time events while retaining canonical provenance, seeds valid pre-battle consumable/Supremacy state at `t=0`, and rejects negative temporal fields at the producer boundary. Type5 loadout decoding now shares the Type32 consumable mapping and covers the reviewed 11.19 food, fuel, protective-kit, and gear-oil wire-code families; unknown codes retain raw values.
