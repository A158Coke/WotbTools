...
### Fixed
- **AI 复盘裸抛 DATASET_UNAVAILABLE 修复（Dataset 状态机 Hotfix）**：`AiReviewPanel` 之前仅凭 `file != null` 就启用「AI 战术复盘」按钮，而 `processingJobId`/`sourceId`（authoritative Dataset 引用）由 `requestDirectAction` 异步补齐，准备期点击即被 `analyzeBody` 裸抛 `DATASET_UNAVAILABLE`。现引入 Dataset 状态机：`file && processingJobId && sourceId` 齐备才允许 Analyze（`datasetReady` 硬 guard），未 READY 时显示「正在准备回放数据…」并禁用按钮；`runAnalyze`/`analyzeBody` 不再把 Dataset 未就绪当作最终用户错误（不发起请求、不显示裸错误码）。同一 selection 的 Parse/AI/战局回放仍共享同一 Processing Job（single-flight 未改动，绝无 multipart AI 重传）。Dataset 过期（`JOB_NOT_FOUND`）时面板 emit `dataset-recover`，`ReplayPage` 清空失效引用并重新走 `requestDirectAction` 自动恢复一次；恢复失败则显示已本地化的「无法准备回放数据，请重试」。内部稳定码（`DATASET_UNAVAILABLE`/`JOB_NOT_FOUND`/`SOURCE_NOT_READY`/`SOURCE_NOT_FOUND`/`DATASET_REFERENCE_REQUIRED`）全部经 `localizeAiError` 本地化，绝不裸展示；`BattlePlaybackPanel` 不再裸设 `DATASET_UNAVAILABLE`（改为准备态/复用 `datasetError`）。`ReplayProcessingJobStore` 为 `@Component`（生产组件扫描必注入 `AiReplayReviewService`），新增 `AiReplayReviewServiceWiringTest` 证明 production 装配注入非空 store。前端全量测试（1275）与构建通过，后端目标单测（62）通过。
