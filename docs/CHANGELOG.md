...
### AI Review
- **Team AI Review technical schema resilience**：Team Call #2 改为一次初始 structured parse + 最多一次定向 repair；parser 返回精确 failure path/category/status，并对 optional episode/player references 做确定性 normalization；新增 schema-failure、normalization、repair 生命周期观测与低基数指标。最终 technical failure 使用独立 `AI_REVIEW_SCHEMA_FAILED`，同步 OpenAPI、生成前端契约、三语文案、诊断 ID、测试与 AI Review 看板；不记录 prompt、completion 或用户级标签。
