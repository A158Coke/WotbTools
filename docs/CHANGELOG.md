...
### Added
- **Evidence limitation 优先级与预算契约**：`TeamAiPromptBuilder` 改为两级输出——required contract section（header + `unitLimitations=[...]`）在前、truncatable feature details 在后；required section 超出 `MAX_INPUT_CHARS` 时 fail fast（`IllegalStateException`），不得静默截断；`BudgetWriter.appendRequired()` 保证 mandatory/global/unit limitations 不因预算丢失；`featureLimitations=` 移除，所有 limitations 在 required section 以 `unitLimitations=` 统一输出；`DATA_LIMITATIONS` 仅含真正全局限制，不混入 per-unit limitation；report 与 prompt 正文 limitation 状态一致。
- **Team 分析入口统一 evidence contract**：`analyzeSingleTeamContext()`/`analyzeMultiTeamContext()` 现与 `analyzeTeamGroups()` 使用相同的 RosterEvidence 派生逻辑；direct methods 内部计算 `RosterEvidence.from()` 或 `rosterLimitationsFromSummary()` 并将 limitations 传入 `TeamAiPromptBuilder.single()/multi()` 的 `extraLimitations`/`evidenceLimitations` 参数；所有入口共享同一 `buildPartitions()` → builder 路径。
- **Enemy-only damage 不参与 Team perspective phase 计算**：`DefaultTeamBattleFeatureExtractor` 确认 enemy-only damage（敌方打敌方/未知实体）不影响 team first contact、phase 边界或 fallback clock；新增 4 个回归测试覆盖空 phase、fallback end、first contact、authoritative duration。
- **Partition 代码清理**：`partitionContexts()` 死委托删除；`contextsCompatible()` 直接使用 `IndexedContext.evidence` 而非 `evidenceByUnitId` map 查找；`canJoinPartition()` 接收 `IndexedContext` 直接避免二次 map 操作；`buildPartitions()` final 结果改用 stream `map()`/`toList()` 简化。
- **PLAYER_FOCUSED prompt injection 边界保护**：新增 `PlayerResultFormat.quoteForPrompt()` 共享 JSON escaping 实现（与 `TeamAiPromptBuilder.quoteData()` 相同），对所有 PLAYER prompt 路径（single full-feature、fallback、multi-player）中的外部字符串应用确定性转义；PLAYER system prompt 添加"不可信数据"安全合约；新增 5 个 injection 边界测试验证 HTTP request body 中的转义。
- **Provider secret redaction 增强**：`safeProviderSummary()` 新增 regex 覆盖 `Bearer credential`、`Basic credential`、`Digest response=secret` 等空格分隔的 scheme+credential 模式；新增 6 个脱敏测试。
- **PLAYER battle end 统一权威 duration 优先级**：`DefaultPlayerBattleFeatureExtractor` 改为与 Team 一致的三级 battle-end 优先级（`battle.durationS` > `BattleEndedEvent` > 录像者最后有效证据）；新增 8 个回归测试覆盖 duration 优先、NaN/负数/空 duration fallback、event fallback、last evidence fallback、其他玩家事件不延长。
- **删除旧 multi-player boolean 路径**：删除无生产调用方的 `analyzeMultiPlayerContext()`、`buildMultiPlayerContextSummary()`、`MultiPlayerBattleAnalysisContext`、`PlayerBattleAnalysisSummary`（含 `boolean victory` 两态压缩）、`MULTI_PLAYER_PROMPT` 常量——所有删除代码已被生产路径 `analyzeMulti(List<Battle>)` + `FriendlyEnemyResult.Winner` 三态取代。
- **清理 `ZERO_CLOCK_INFERRED` 不可达枚举**：删除 `BattleStartResolution.Status.ZERO_CLOCK_INFERRED` enum、`zeroClockInferred()` factory 及对应 constructor 分支——无可达生产路径返回该状态。
- **唯一共享 PromptDataQuoter**：新建 `com.wotb.core.util.PromptDataQuoter`，包含唯一 JSON escaping 实现；`PlayerResultFormat.quoteForPrompt()` 和 `TeamAiPromptBuilder.quoteData()` 改为委托调用，删除重复 escaping switch。
- **共享 BattleEndResolver**：新建 `com.wotb.core.replay.feature.BattleEndResolver`，统一 Player/Team battle-end duration 验证（`> 0`）、event 和 scope-local fallback；Player unknown 时添加 `BATTLE_END_UNRESOLVED` limitation；Team duration 检查由 `>= 0` 改为 `> 0`。
- **Multi-team 两阶段预算分配**：`TeamAiPromptBuilder.multi()` 改为预计算所有 perspective required block 字节 → 确定性缩减 → 写入的两阶段流程；超过 MAX_PERSPECTIVES 或预算无法容纳时添加 `PERSPECTIVES_OMITTED_COUNT_N` limitation；`appendRequired()` 抛出 `AiUpstreamException("AI_PROMPT_TOO_LARGE")`；删除 `finish()` 中 `setLength()` 整体截断。
- **Authorization scheme 通用脱敏**：`redactNonJson()` 支持任意 CamelCase auth scheme（`CustomScheme`、`TokenV2`、`ApiAuth` 等）；Digest 字段覆盖 `uri` 和 `username`。
- **AI Review 16 文件上传限制**：`ReconstructionController.MAX_AI_REVIEW_REPLAY_FILES = 16`；前端 `ReconstructionPage.vue` 和 `ReplayFilePicker.vue` 增加 16 文件硬限制；i18n 三语提示和计数显示；后端 error code `REPLAY_FILE_COUNT_EXCEEDED`。
- **Omitted perspective 不再报告为已分析**：`PromptInput` 扩展为包含 `includedUnitIds`/`omittedUnitIds`/`perUnitLimitations`；`analyzeTeamGroups()` 只给 included unit 分配 AI 结果，omitted unit analysis/model 为 null，添加 `AI_PERSPECTIVE_OMITTED_FROM_PROMPT`；`analyzedUnitCount` 从 true included count 派生；Controller 使用真实计数。
- **Team 接入共享 BattleEndResolver**：删除 `DefaultTeamBattleFeatureExtractor` 中 `findBattleEndEvidence()`/`BattleEndEvidence`；调用 `BattleEndResolver.resolve()`；`lastObservedClock()` 返回 nullable；新增 `BattleEndSource` enum；`BattleEndResolverTest` 独立测试 17 项。
- **三层 Prompt Budget 优先级**：`single()` context header 全用 `appendRequired()`；`appendFeatureSet()` 拆为 P2 facts（authoritative/observed/memberFacts/coverage）→ P3 optional（movements/formation/phases/engagements/events）；`appendMembers()` 拆为 `appendMemberFacts()` + `appendMemberMovements()`；本地 budget 错误使用 `IllegalStateException` 而非 `AiUpstreamException`。
- **通用 Authorization Scheme 脱敏**：分层 regex：已知 scheme（大小写不敏感）→ PascalCase custom scheme → 含数字/标点的 lowercase scheme；新增 14 个测试；反误报自然语言测试。
...
- **坐标三态与战斗开始解析**：新增 `BattleStartResolution`（IDENTIFIED/ZERO_CLOCK_INFERRED/ESTIMATED/UNRESOLVED，含严格不变量的 static factories）、`BattleStartResolver`；`DefaultTeamBattleFeatureExtractor` 使用 `battleRelative()` 计算 formation window、first contact；`DefaultPlayerBattleFeatureExtractor` 使用 `isPreBattle()` 过滤准备阶段位置/伤害事件；坐标解析统一为 `MapRegionResolver.resolve()` 返回 `MapCoordinateResolution`（VALID/CLAMPED/INVALID）；`CLAMPED` 位置计入 `clampedPositionEventCount` coverage 并添加 `MAP_COORDINATES_CLAMPED` limitation；`TeamFeatureCoverage` 增加 `clampedPositionEventCount`字段；`isOutOfBounds`/`isClamped` 使用共享 resolver 而非独立 `Math.abs()`；tolerance `CLAMP_TOLERANCE_RAW=50`（12.5 canonical meters，2.5% of MAP_SIZE）。
- **MemberIdentity accountId=0 昵称唯一性**：`MemberIdentity.resolve(PlayerResult, List)` 作为唯一构造入口；`accountId<=0` 且 blank/null 昵称标记为 `ambiguousNickname=true` 不抛异常；重复/空白/大小写冲突昵称不匹配任何 entity；`buildMember()` 对 ambiguous 成员添加 `TEAM_MEMBER_IDENTITY_UNRESOLVED` limitation；删除旧两参数构造器和 `findBattleStart` 死方法。
- **Authorization 与 API key 脱敏强化**：`SENSITIVE_KEYS` 精确集合替换为 `isSensitiveKey()` 归一化匹配（大小写、连字符、下划线前缀）；新增 x-api-key、AWS Access Key、AWS Secret Access Key、client_secret、security token、Credential/Signature 正则覆盖；JSON tree redaction 递归扫描 string value 内容；增加 12 个 security 测试（含 x-api-key、AWS、mixed-case、log capture via call() 异常路径）。
- **方法入参 final 化**：`DefaultPlayerBattleFeatureExtractor.extract`/`compressMovements`、`BatchAnalyzer.resolveKey`、`ReconstructionController` 构造器及 handler 参数、`DefaultBattleFeatureExtractor` 多方法参数补齐 `final`。`DefaultTeamBattleFeatureExtractor.buildKeyEvents`/`buildTeamEngagements` 改为 `private static`。
- **AI 战术复盘（DeepSeek，`wotbtools-admin` 灰度）**：新增 `POST /api/replay/analyze` 与 `AiReplayAnalysisService`（Spring `RestClient` 调 OpenAI 兼容 `/chat/completions`）。**以 `battle_results.dat`（`Battle`/`PlayerResult`）为权威数据源**——伤害/承伤/助攻/格挡/击杀/存活/死亡时刻(`deathTimeMillis`)/录像者均取自游戏结算；死亡时间线据此生成；完整重建仅补充位置维度。经统一门面 `DefaultReplayProcessingFacade.process(full)` 获取战绩+重建，战绩失败返回 `NO_BATTLE_DATA`、战绩成功但重建失败仍可分析、未配置密钥返回 `AI_NOT_CONFIGURED`（应用照常启动）。密钥 `AI_API_KEY` 经 GitHub Actions secret → 容器环境变量注入（沿用 `KEYCLOAK_ADMIN_CLIENT_SECRET` 路径）。管理员选择回放后可直接发起分析，不依赖先展示重建结果；结果面板支持单/多玩家与单/多团队模式。
...
- **AI 上游错误与前端本地化**：将 400/401/403/408/413/422/429/5xx、读超时、空响应、畸形 JSON 和非法响应映射为稳定英文错误码；日志只记录模型、状态、请求字符数、模式、关联 ID 与脱敏限长摘要，不记录 API Key、Authorization 或完整 Prompt。前端 zh/en/ru 只展示本地化错误码，未知后端文本不会直接暴露给普通用户。
