...
- **敌方点亮仍半透明（lastKnown 语义）**：vehicleState.lastKnown 由 `!live || !covered` 改为 `!covered`——route 采样间隔（max(2s, duration/200)，长局可 >5s）导致 live=null 不代表位置中断；位置流覆盖（covered）即「点亮」，只有未覆盖才淡化。原「gap 淡化」测试改为覆盖=false 场景，新增「covered=true 且采样 gap>5s 不淡化」回归。
...
- **双方总血量条 + 争霸赛点数（playback）**：MapOverviewBuilder.buildPlayback 消费 HealthChangedEvent（propId=3，EXACT）→ `PlaybackVehicle.maxHp`（`ObservedMaxHp` 解析）+ `hpSamples`（battle-relative 秒升序）；`MapOverview` 增 `friendlyPoints/enemyPoints`（victoryPointsEarned 按队求和，非争霸赛 null）；前端 BattlePlayback 新增两条总血量条（`utils/battlePlayback.js` 纯函数 `vehicleHpAt/teamHp`，随播放实时下降）+ 争霸赛点数；locale `recon.map.playback.points` 三语。
