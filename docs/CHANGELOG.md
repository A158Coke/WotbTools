...
- **TX RabbitMQ provider mirror 自举**：`services=rabbitmq` 现在在 `tofu init` 前从 committed RabbitMQ lockfile 推导 `cyrilgdn/rabbitmq 1.10.1`，用 OpenTofu provider mirror 命令安装精确的 `linux_amd64` 包到 `/opt/wotb-tx/tofu-provider-mirror/registry.opentofu.org/cyrilgdn/rabbitmq`；已有精确包幂等复用，版本、checksum 或安装失败均 fail closed，正式 init 继续只允许 filesystem mirror 且保留 second-plan no-changes 硬门禁。Keycloak/PostgreSQL provider 路径与 RabbitMQ topology ownership 不变。
