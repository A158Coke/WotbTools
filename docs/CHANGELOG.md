...
### Added
- **回放单文件上限 20MiB → 5MiB**：`ReplayUploadValidator.MAX_FILE_SIZE` 收紧为 5MiB（WoT Blitz 真实回放 1~3MiB；百场持久化 5 个原始回放，单文件 20MiB 时单申请最多 100MiB 的磁盘风险）；覆盖名人堂上传 / AI 复盘 / 批处理 / 百场提交；`spring.servlet.multipart.max-file-size` 同步 5MB；AI 复盘相关测试改用常量自适应。
