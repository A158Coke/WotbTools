...
- **TX secret injection boundary**：TX Compose 与 TX-local OpenTofu 仅接收 GitHub Actions Secrets/Variables 通过 SSH 注入的进程环境；不再依赖服务器 `/etc/wotb/*.env` 文件。新增 TX preflight 检查 GitHub 输入、Docker/Compose、OpenTofu、`wg0` 地址与 WireGuard 路由，缺失变量时 fail-closed 且不打印 secret；backend `8087` 动态可达性只在 deploy/PRE_CUTOVER_READY runtime gate 验证。
