...
- [x] PR #264 review blocker：由 authoritative Team roster 提供 `teamPlayers` 映射，贯通 SSE/OpenAPI/生成契约/runtime guard/UI/copy，并覆盖缺失映射 fallback。
- [x] review blocker 修复 commit：`efb2ab89`；PR：[#264](https://github.com/A158Coke/WotbTools/pull/264)。
...
- 验证：`npm run api:lint`、`npm run api:generate`、`npm run typecheck`、`npm test`（114 files，1579 passed，37 skipped）、`npm run build`、`mvn -q -s settings.xml -pl wotb-web -am test`。
- 范围外：真实 AI provider smoke、真实回放上传/生产写入；需显式用户请求。
- PR：[#264](https://github.com/A158Coke/WotbTools/pull/264)
