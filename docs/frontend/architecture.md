...
Playback tests follow those ownership boundaries: component contracts live in `BattleMap.test.js`, `PlaybackControls.test.js`, `PlaybackTimeline.test.js`, and `VehicleDetailsPanel.test.js`; `BattlePlayback.test.js` is reserved for small orchestrator integration checks, while the broader legacy regression coverage is retained as `BattlePlayback.integration.test.js`.
