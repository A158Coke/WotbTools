...
- `frontend/src/composables/useReplayWorkspace.js` / `useReplay.js` 持有 selection、当前 battle 与 Processing Job；Workspace 向下提供这份基础状态。
...
- 多文件选择、当前 battle 选择和 capability 切换都由 Workspace 协调；单一 reconcile 根据当前 selection、source identity 和 Processing 状态准备活跃 capability。
