...
- CW（训练房/联赛）恒为争霸赛：result 与 resultSource 三级证据（BATTLE_RESULTS 权威 / SURVIVOR_SETTLEMENT 结算存活推导 / POINTS_INFERENCE 非权威推断）；全歼双向（全歼敌方获胜 / 被敌方全歼落败），双方均有存活才按点数结束（≥1000 提前 / 双方 <1000 时间耗尽）；集中一波的代价 = 丢视野 + 被偷家；残局守家 vs 占点决定点数胜负；占点分是权威总量，不是时间线。
