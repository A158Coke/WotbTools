...
- **正确判定（LLM 解释）**：可判**拖延**（可观测行为：静止/卡点 + 敌情压力）；「个人复盘无队友获利维度」只说明 Backend 不输出该字段，不是要求 LLM 证明或否认它。
...
- **规则引用**：player `SEPARATION_EVIDENCE_RULE`（Backend 只给中性测量；拖延与否由 LLM 综合判断，不得说成心理意图）。
