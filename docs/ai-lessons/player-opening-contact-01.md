...
- **判定依据**：`PlayerSeparationEvidenceSkill.classify` 用窗口内重叠 engagement 的 `damageDealt/damageReceived` 计算 `contactObserved`，开局窗口内有接触 → 空候选。
- **对应 golden case**：`ai-eval/cases/player-opening-contact-01.json`
- **规则引用**：player `SEPARATION_EVIDENCE_RULE`（开局分散 = 未接火未承伤未阵亡）。
