...
- **判定依据**：`PlayerSeparationEvidenceSkill.classify` 对 `OBSERVED_DAMAGE_IS_PARTIAL` 抑制开局分散（与 Team 路径一致）。
- **对应 golden case**：`ai-eval/cases/player-damage-partial-opening-01.json`
- **规则引用**：player `SEPARATION_EVIDENCE_RULE`（未接火未承伤未阵亡才可判开局分散，覆盖不完整时无法确定）。
