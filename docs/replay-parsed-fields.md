...
| `rosterComplete` | Boolean | 结算阵容完整性证据：名册(#201)与战绩(#301)账号集合一致（所有参战成员都有结算记录）且名册提供队伍(#201→#2→#3)时与结算队伍一致时为 true；null=未知/不完整（来源：`ReplayParser` 对比 #201/#301）。只有为 true 才允许用 survivors==0 断言全歼或推导 SURVIVOR_SETTLEMENT |
