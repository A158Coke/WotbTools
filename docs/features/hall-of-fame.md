...
- 管理后台列表可按国家/系别、车种、具体车辆和状态独立筛选（百场仅 Tier X，不另设等级）；摘要只显示认证场均和认证场次：WG 使用冻结的官方快照，MANUAL 使用管理员通过后的数值；用户申报值仅在详情中查看。对 PENDING / CURRENT / REJECTED / SUPERSEDED / CANCELLED / DELETED 一律只提供“详情”入口。通过、拒绝、删除只能在详情内触发，管理员不提供成绩编辑控件；截图、回放列表与下载按钮只在 PENDING 详情展示，终态详情只保留审核结果、拒绝/删除原因等文字信息。
