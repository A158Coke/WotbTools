...
- **type 32 历史 runtime-double 解释已 SUPERSEDED**：完整 34-arena strict corpus 证明结构为 `entityId:u32 + flag:u8 + bodyLength:u32 + body`，且 `bodyLength == remaining bytes` 为 16,850/16,850、零反例。旧探针固定偏移读取的所谓 `double` 跨越不同长度的 variable body，不构成字段。Type32 同时覆盖 Type5 mobile 与 static entity family；安全结论仅为实体级 length-prefixed auxiliary blob，body 语义待续。
