...
- **type 32 全局 runtime-double 解释已 SUPERSEDED，但子布局 double 已闭环**：完整 34-arena strict corpus 证明 outer structure 为 `entityId:u32 + flag:u8 + bodyLength:u32 + body`，且 `bodyLength == remaining bytes` 为 16,850/16,850、零反例；Type32 同时覆盖 Type5 mobile 与 static entity family。按 class/flag/length 分流后，mobile `flag=0` 15/16B body 的正确尾部结构为 `float64 eventClock + float32 param`：2,892 个非零 double 在 34/34 replay 内满足 `eventClock-rawClock≈场内常量`，最大 spread 0.312s。其 16B active/control form 进一步闭环消耗品状态：body[2] wire code、body[3] state 1/2/3/255，已按 duration/cooldown 与 item catalog 映射 Adrenaline、Engine Power Boost、Multi-Purpose Restoration Pack、Improved Engine Power Boost、Reticle Calibration、Reactive Armor、Tungsten Shells；0x0C/0x0D 仅能锁定为 First Aid/Repair pair，精确互换仍 UNKNOWN。
