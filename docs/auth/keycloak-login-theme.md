...
- 主题切换：右上 pill 内用 CSS 绘制 Sun（圆+8 射线）与 Moon（月牙）双图标，双图标常显、当前主题态高亮另一态置灰；aria-label/title/data-label-* 中文硬编码，切换后 JS 同步指向「下一目标主题」；dark/light 两主题均有 `:focus-visible`。
- Locale：登录页不提供语言选择器；主题自创文案（theme toggle aria/tooltip）中文硬编码；登录表单字段仍由 Keycloak 基础包按浏览器语言本地化。
- Dark readability：深色登录卡自带局部毛玻璃承载面；`.wbtb-shell__auth::before` 软径向 dark veil 保留（弱化卡片四周背景，无硬矩形/左右分区）+ 提升 input/eye/IdP/divider 对比（dark-only），light 零回归。
