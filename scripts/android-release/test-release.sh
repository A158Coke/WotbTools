...
resolve workflow_dispatch | grep -q '^versionName=1.4.4$' || fail "committed version authority"
resolve workflow_dispatch | grep -q '^versionCode=1004004$' || fail "versionCode formula"
resolve workflow_dispatch | grep -q '^nativeBridgeVersion=1$' || fail "bridge version from contract"
resolve push android-v1.4.4 | grep -q '^tagName=android-v1.4.4$' || fail "compatible tag"
