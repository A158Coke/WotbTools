 ...
 */
export function useExportJob(session: Session, auth: ReplayAuthSession) {
  const { t, te } = useI18n()
  ...
  async function poll(ownerToken: number, currentJobId: ExportJobId): Promise<void> {
    ...
    try {
      const data = await api.getExportJob(auth, currentJobId)
  ...
  async function start(mode: ExportMode, teamNamesOverrides: JsonObject | null = null): Promise<void> {
    ...
    try {
      ...
      const created = await api.createExportJob(auth, mode, processingJobId.value, teamNamesJson)
      if (ownerToken !== lifecycleToken) {
        api.cancelExportJob(auth, created.jobId).catch(() => {})
        return
      }
  ...
  async function cancel(): Promise<void> {
    ...
    const ownerToken = lifecycleToken
    try {
      await api.cancelExportJob(auth, job.jobId)
  ...
  async function download(): Promise<void> {
    ...
    try {
      ...
      await api.downloadExportJob(auth, job.jobId, job.filename || fallback)
      if (ownerToken !== lifecycleToken || exportJob.value?.jobId !== job.jobId) return
    } catch (e) {
      if (ownerToken !== lifecycleToken || exportJob.value?.jobId !== job.jobId) return
      exportError.value = apiErrorLabel(t, te, e)
