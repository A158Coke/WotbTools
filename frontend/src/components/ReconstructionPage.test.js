...
describe('ReconstructionPage 跨视图文件接管（V2：replayTransfer 登录后/KeepAlive/连续接管）', () => {
  function mapOverviewFixture() {
    return {
      mapCode: 'desert_train',
      displayName: 'Desert Sands',
  ...
  function mapStub(seen) {
    return {
      ...
      props: ['overview', 'seekTo'],
      emits: ['seek'],
      template: '<div class="map-stub" data-test="map-stub" />',
      mounted() { seen && seen.push(this.$props.overview) }
    }
  }
  beforeEach(() => {
    auth.ensureToken.mockResolvedValue(true)
  ...
  afterEach(async () => {
    vi.unstubAllGlobals()
    vi.useRealTimers()
    // 清空模块级 replayTransfer，避免跨测试污染
    const transferModule = await import('../utils/replayTransfer.js')
    transferModule.takePendingReplayFiles()
  })

  async function preloadTransfer(fileName, mode) {
    const transferModule = await import('../utils/replayTransfer.js')
    const file = new File(['replay'], fileName, { type: 'application/octet-stream' })
    transferModule.setPendingReplayFiles([file], mode)
  }

  it('已登录挂载时接管 pending 文件：playback 模式自动加载地图（/api/replay/map-overview）', async () => {
    const seen = []
    vi.stubGlobal('fetch', vi.fn().mockResolvedValue({
      ok: true,
      status: 200,
      json: vi.fn().mockResolvedValue(mapOverviewFixture())
    }))
    await preloadTransfer('lagoon.wotbreplay', 'playback')
    const wrapper = mount(ReconstructionPage, {
      global: { mocks: { $t: i18n.t }, stubs: { MapOverview: mapStub(seen) } }
    })
    await flushPromises()
    // 文件被接管并显示在 chips
    expect(wrapper.text()).toContain('lagoon.wotbreplay')
    // playback 模式自动请求地图（不经用户点按钮）
    const call = fetch.mock.calls.find(([url]) => String(url) === '/api/replay/map-overview')
    expect(call).toBeDefined()
    // AI 复盘未自动发起（无 /api/replay/analyze 请求）
    expect(fetch.mock.calls.some(([url]) => String(url) === '/api/replay/analyze')).toBe(false)
    // 地图视图挂载
    expect(wrapper.findComponent({ name: 'MapOverview' }).exists()).toBe(true)
  })

  it('已登录挂载时接管 pending 文件：ai 模式只接管文件，不自动加载地图、不发起 AI', async () => {
    vi.stubGlobal('fetch', vi.fn())
    await preloadTransfer('lagoon.wotbreplay', 'ai')
    const wrapper = mount(ReconstructionPage, {
      global: { mocks: { $t: i18n.t }, stubs: { MapOverview: true } }
    })
    await flushPromises()
    expect(wrapper.text()).toContain('lagoon.wotbreplay')
    // 无任何自动请求
  ...
  it('KeepAlive 返回（onActivated）时接管新 pending 文件', async () => {
    vi.stubGlobal('fetch', vi.fn())
    // 用 KeepAlive 包裹 ReconstructionPage，模拟 App.vue 的 ?view= 切换
    const Harness = {
      components: { ReconstructionPage },
      template: '<KeepAlive><component :is="page" /></KeepAlive>',
      data: () => ({ page: ReconstructionPage })
    }
    const wrapper = mount(Harness, {
      global: { mocks: { $t: i18n.t }, stubs: { MapOverview: true } }
    })
    await flushPromises()
    // 切走（unmount 页面实例但仍缓存）→ 切回触发 onActivated
    wrapper.vm.page = { template: '<div />' }
    await flushPromises()
    await preloadTransfer('alpha.wotbreplay', 'ai')
    wrapper.vm.page = ReconstructionPage
    await flushPromises()
    const recon = wrapper.findComponent(ReconstructionPage)
    expect(recon.text()).toContain('alpha.wotbreplay')
  })

  it('连续接管不同回放：每次 take 最新 pending（不重复、不覆盖旧会话残留）', async () => {
    vi.stubGlobal('fetch', vi.fn())
    const transferModule = await import('../utils/replayTransfer.js')
    const f1 = new File(['replay'], 'first.wotbreplay', { type: 'application/octet-stream' })
    const f2 = new File(['replay'], 'second.wotbreplay', { type: 'application/octet-stream' })
    // 连续两次 setPending（模拟用户连续点不同 battle）
    transferModule.setPendingReplayFiles([f1], 'ai')
    transferModule.setPendingReplayFiles([f2], 'playback')
    const wrapper = mount(ReconstructionPage, {
      global: { mocks: { $t: i18n.t }, stubs: { MapOverview: true } }
    })
    await flushPromises()
    // take 语义：只取最新一次（second，playback）；不因两次 set 都接管
    expect(wrapper.text()).toContain('second.wotbreplay')
    expect(transferModule.takePendingReplayFiles()).toBeNull()
  })

  it('未登录（登录回跳前）不接管 pending；登录后 onActivated 再接管', async () => {
    authState.authenticated.value = false
    vi.stubGlobal('fetch', vi.fn())
    await preloadTransfer('lagoon.wotbreplay', 'ai')
    const wrapper = mount(ReconstructionPage, {
    ...
    await flushPromises()
    // 未登录：initPromise resolve false → login 分支，不渲染内容
    expect(wrapper.text()).toContain('recon.pleaseLogin')
    // pending 仍保留（未消费）
    const transferModule = await import('../utils/replayTransfer.js')
    expect(transferModule.peekPendingReplayFiles()).toBeTruthy()
  })
})