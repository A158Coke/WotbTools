...
describe('HomePage information architecture — single replay entry point', () => {
  ...
  it('quick links panel is preserved', () => {
    ...
    expect(quick.findAll('a')).toHaveLength(5)
    const androidLink = quick.findAll('a').find(a => a.attributes('href') === '/download/android')
    expect(androidLink).toBeTruthy()
    expect(androidLink.text()).toContain('android.nav')
    wrapper.unmount()
  })
})
