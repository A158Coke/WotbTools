...
- `build.yml`：在 main 的应用或运行时 observability 变更后构建并推送 SHA 镜像，也支持 `workflow_dispatch` 单独选择 backend/frontend/keycloak/all；纯 `deploy/observability/grafana/dashboards/**` 只走 Grafana OpenTofu，不触发 Build。**不运行 backend/frontend/integration 测试套件**（测试与 merge 验证由 PR CI 承担）。
