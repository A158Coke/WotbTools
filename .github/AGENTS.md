...
- `benchmark-tcr.yml`：仅 `workflow_dispatch` 的直接发布 PoC benchmark——用 BuildKit registry exporter 把真实 `docker/Dockerfile.backend` **一次构建直接推送到 TCR** 的 `benchmark-<run id>` 标签，只测量 GitHub-hosted Runner → TCR 这一段网络路径。它不接力 Deploy、不产出 `deployment-manifest`、不经 GHCR/OCI tar/rsync/SSH/`docker load`、不使用 `latest` 或 `sha-<12>` release identity、也不挂 `environment`（TCR 凭据是仓库级 secret）；`scripts/ci/test-workflow-contract.sh` 固定这些边界。生产发布链仍是唯一 release 路径。
