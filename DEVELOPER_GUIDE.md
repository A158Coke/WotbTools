...
└── java/
    ...
    │   │   ├── ReplayController.java    # REST API（仅 HTTP 映射）
...
- 导出层：`Columns.java`（单场 xlsx 表头）、`ExcelExporter` 的汇总列。
...
- 列：单场「评分」`key=rating`(在 `Columns.STAT`)、汇总「场均评分」`key=rating_avg`(Mapper/ExcelExporter)。计算时机：`ExcelExporter.writeSingle/writeAggregate` 与 `ReplayService.preview` 在用之前先 `Rating.compute(...)`。
