...
| `GlobalExceptionHandler` | `wotb-web/.../controller/GlobalExceptionHandler.java` | 统一异常处理 → JSON 错误响应（3 字段） |
...
| `ErrorCode` | `wotb-web/.../dto/ErrorCode.java` | 通用 API 错误码枚举 |
...
- `BoostPage.vue` 的打手管理页同时展示两套状态：`booster_profile.status` 是资格状态（`ACTIVE/INACTIVE/BANNED`），`booster_profile.available` 是接单状态（可接单/暂停接单）。改动任一含义时，务必同步三语 locale，避免再次出现“正常 + 不可用”这类歧义。
...
- **Schema 来源**：Flyway 迁移 `wotb-web/.../resources/db/migration/V1__init_leaderboard.sql` → `V9__create_booster_application.sql`。**改表结构必须新增迁移**（`V3__...`），不要改已应用的 V1/V2；实体列与迁移列**逐列对齐**，否则 `validate` 启动即失败。
- **打手资格申请**：`booster_application` 保存玩家申请的 WoTB 账号、两张截图、申请等级、QQ/微信、可接单频率、日在线时间和审核状态；同一 Keycloak 用户只允许存在一个 `NEW`/`REVIEWING` 申请。审批通过时先调用 Keycloak Admin API 授予 `booster` role，再创建 `booster_profile`。
- **打手资料双状态**：`booster_profile.status` 控制资格是否有效；`booster_profile.available` 控制当前是否允许接单。分配打手时两者都要满足（`ACTIVE` 且 `available=true`）。
