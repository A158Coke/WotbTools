...
- **车辆库**：`tankopedia-tier{7,8,9,10}.json`（4 个文件，无 `tankopedia.json`）。数据源 BlitzKit production definitions（`api.blitzkit.app/definitions/*.pb`）；`.github/workflows/update-tankopedia.yml` 与本地 `cd common/python && python update_tankopedia.py` 共用同一 production API source of truth；写入前有完整性门禁（总量/tier 骤降、重复 id、缺 id/name/hp/gun 即失败）。
