...
2. **跨层一致性。** 列的 `key`(snake_case)在 **API / 前端 / 导出** 三方一致;显示名在 **前端三语 locale(`frontend/src/locales/{zh,en,ru}.json` 的 `player_labels`/`agg_labels`)+ 导出两处(`Columns.java`、`AggregateSheets` 汇总列)** 一致。
...
- 不要在 `target/`、`node_modules/`、`dist/`、`.m2repo/` 里改源码。
