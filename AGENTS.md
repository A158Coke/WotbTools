...
项目:从 WoT Blitz `.wotbreplay` 提取战斗数据导出 Excel。`java/`(主线:`wotb-core` 共享核心 + `wotb-web` Spring Boot + `frontend` Vue;`offline/` jpackage、`online/` docker)、`common/`(共享 tankopedia/icon/样本)、`common/python/`(车辆库更新脚本)。CI/CD: `.github/workflows/deploy.yml` push main 触发,构建根 `Dockerfile` 单镜像 → Docker Hub(`a158coke/wotbtool`) → SSH 部署 VPS(`/opt/wotb`)。
