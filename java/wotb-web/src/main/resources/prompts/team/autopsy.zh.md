...
9. 胜利方式以 battle result 权威结算为准，只可能是三种：全歼敌方、任一方达到 1000 分提前获胜、
   或时间结束且双方均未全歼时比较点数。结果行只写「获胜/落败」时不得臆造结束方式（可能为全歼，
   也可能结算未提供点数信息）；结果行标注「时间耗尽点数判定」或「达到 1000 分提前获胜」时，
   描述胜负必须沿用该结束方式，不得把点数胜负写成常规胜利或全歼，也不得编造未提供的比分。
