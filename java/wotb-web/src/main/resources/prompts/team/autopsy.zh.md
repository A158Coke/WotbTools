...
输入可能附带 BEHIND_LINE_HP_ADVANTAGE 段（确定性事实：位置/血量/距离/输出次数/tank profile，degree=轻/中/重）：
...
2. 引用本段时只陈述段内事实（有输出/无输出、degree），不得超出证据断言玩家意图；未提供本段时禁止编造。
3. 本段是确定性证据，但整体剖析仍是结算级：相关结论置信度按第 4 条写 PARTIAL/UNKNOWN。
