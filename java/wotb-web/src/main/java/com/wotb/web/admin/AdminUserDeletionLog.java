...
/** 管理员用户删除审计日志。记录谁删了谁、结果如何。 */
@Entity
@Table(name = "admin_user_deletion_log")
public class AdminUserDeletionLog {
    ...
    public AdminUserDeletionLog() {}

    // ── 工厂方法 ────────────────────────────────────────────────────

    public static AdminUserDeletionLog started(
            final String targetKeycloakUserId,
            final com.wotb.web.user.entity.UserProfile profile,
            final String adminKeycloakUserId,
            final String adminUsername) {
        final AdminUserDeletionLog log = new AdminUserDeletionLog();
