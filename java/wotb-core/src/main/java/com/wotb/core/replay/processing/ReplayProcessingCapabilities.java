 ...
 * <p>
 * scope 可分析规则由 {@link #aiAnalyzable(ReplayAnalysisScope)} 统一计算（单一 SSOT），
 * 不在本 record 中预计算为字段。
 * </p>
 *
...
public record ReplayProcessingCapabilities(
    ...
    /**
     * scope-aware AI eligibility（单一 SSOT）：由当前 result 的实际 capability facts 即时判定，
     * 不做任何缓存/预计算。null scope 一律不可分析。
     */
    public boolean aiAnalyzable(final ReplayAnalysisScope scope) {
        if (scope == null) {
            return false;
        }
        return switch (scope) {
            case PLAYER_FOCUSED -> summaryAvailable() && recorderResultAvailable();
            case TEAM_PERSPECTIVE -> summaryAvailable()
                    && perspectiveTeamResolved()
                    && (recorderResultAvailable()
