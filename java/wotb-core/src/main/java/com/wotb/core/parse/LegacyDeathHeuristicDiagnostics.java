...
/**
 * Legacy 死亡启发式诊断（§B2/B3 重命名）：EntityLeave / Position 停止更新 / 逐次伤害阈值
 * 三条证据链的死亡时刻估算。
 *
 * <p><b>非权威</b>：这些启发式不再是 PlayerResult 死亡 authority（Type4 != death、
 * last position != death、damage threshold != death，§13 contracts）。
 * 仅保留给 diagnostics / research / 既有测试使用；production DTO 不得调用。</p>
 */
final class LegacyDeathHeuristicDiagnostics {

    private LegacyDeathHeuristicDiagnostics() {
    }
