...
- **百场（Hundred Battles）**：Tier X 车辆独立的生涯场均伤害排行榜，所有用户统一使用人工证据审核（`com.wotb.web.hundred` 域）：
  ...
  - `POST /api/hof/hundred/submissions/{id}/cancel` — 用户撤销自己的 PENDING（**需登录**）。归属按当前登录用户绑定的 WotB 账号判定：账号不匹配或未绑定账号 → 403 `HUNDRED_FORBIDDEN`；记录创建后改绑到别的账号时记录仍属原账号，不可再取消。
  - `GET /api/users/hundred/status` — 个人中心百场状态（CURRENT / PENDING / 最近拒绝；**需登录**）；按当前绑定的 WotB 账号查询，未绑定账号时三个列表均为空。
  ...
  - 数据模型：`hundred_battle_submission` 单表生命周期（Flyway `V18`；`V22` 把 ownership 由 Keycloak 身份改为 WotB 账号），partial unique index 保证 **WotB 账号 + vehicle** 最多一个 PENDING/CURRENT（`uk_hundred_battle_pending_account_vehicle` / `uk_hundred_battle_current_account_vehicle`，`V22` 前为 `..._user_vehicle`）；榜单 owner 列 `wotb_account_id`（`V22` 前名为 `game_account_id_snapshot`），`user_keycloak_id` 已删除。V20 的历史 source/snapshot 列保留为 retired schema residue，不再由应用映射。proof 截图和 5 个原始 replay 仅 PENDING admin-only 保留，终态立即清理。
...
- **三环（Mark 3）**：Tier X 单车最速三环排行榜，仅走人工审核（`com.wotb.web.mark3` 域）：
  ...
  - `POST /api/hof/mark3/submissions/{id}/cancel` — 用户撤销自己的 PENDING（**需登录**）。归属按当前登录用户绑定的 WotB 账号判定：账号不匹配或未绑定账号 → 403 `MARK3_FORBIDDEN`。
  - `GET /api/users/mark3/status` — 个人中心三环状态（CURRENT / PENDING / 最近拒绝；**需登录**）；按当前绑定的 WotB 账号查询，未绑定账号时三个列表均为空。
  ...
  - 数据模型：`mark3_submission` / `mark3_replay_evidence`（Flyway `V21`；`V22` 把 ownership 由 Keycloak 身份改为 WotB 账号）；状态仅 PENDING/CURRENT/REJECTED/CANCELLED/DELETED，禁止 `SUPERSEDED` 和任何 CURRENT 替代。同 **WotB 账号**同车已有 CURRENT 时拒绝新提交/通过；REJECTED/CANCELLED/DELETED 后允许重提。截图与 5 个回放仅在 PENDING admin-only 保留，所有终态立即清理。
