...
- `POST /api/leaderboard/upload` — 上传单场回放（**需登录**）；非随机战（训练房/娱乐/联赛/未知）→ 400 `NON_RANDOM_BATTLE`；其余跳过时返回英文 `reasonCode`（`DUPLICATE_OR_UNKNOWN_RECORDER` / `REPLAY_HASH_CONFLICT`），由前端本地化。
