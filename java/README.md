...
- 车辆库单一来源在 `common/tankopedia.json`（由 `common/python/update_tankopedia.py` 从 blitzkit 游戏客户端数据同步，`vehicles` 数组全英文格式，含手工 `extraInfo` 每车知识点与 `priority`；默认只保留 7–10 级）；`wotb-core` 构建时自动复制到 classpath，勿在模块内再放副本。
