...
- **并发同 identity**：per-operation reservation（`CompletableFuture`）保证唯一 creator 创建并提交、其余 duplicate 等待同一 future；identity 只在 `dispatcher.submit` **成功之后** publish，因此 creator 失败（如 `PROCESSING_QUEUE_FULL`）时两个 caller 一起失败，绝不返回随后被清理的 jobId，也不会各自 submit 出两个 job；失败后 reservation / committed identity / doomed job 与临时存储全部清理，后续同 identity 请求可重新创建有效 job。不使用全局锁。
