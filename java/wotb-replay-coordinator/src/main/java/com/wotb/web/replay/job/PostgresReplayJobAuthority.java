...
/**
 * {@link ReplayJobAuthority} 的 **PostgreSQL 实现**（生产唯一实现）。
 *
 ...
 *
 * <p><b>读取一致性</b>：{@link #findJob} 是两条 SELECT（job 行 + source 行），REPEATABLE READ
 * 保证它们看到同一个已提交快照，不会读出「新 job 状态 + 旧 source 行」。</p>
 *
 ...
 */
public final class PostgresReplayJobAuthority implements ReplayJobAuthority {

    private static final Logger LOGGER = LoggerFactory.getLogger(PostgresReplayJobAuthority.class);

    private final JdbcClient jdbc;
    /** 写入事务：job 行与 source 投影必须一起提交或一起回滚。 */
    private final TransactionTemplate writeTx;
    /** 读取事务：{@link #findJob} 的两条 SELECT 必须在同一快照内完成。 */
    private final TransactionTemplate readTx;

    public PostgresReplayJobAuthority(final JdbcClient jdbc,
